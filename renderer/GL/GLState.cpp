...
namespace Shit
{
	...
	{
		if (mDrawFramebufferState.buffer == framebuffer)
			mDrawFramebufferState.buffer = 0;
		if (mReadFramebufferState.buffer == framebuffer)
			mReadFramebufferState.buffer = 0;
	}
	...
	void GLStateManager::EnableCapabilityi(GLenum, GLuint)
	{
		ST_THROW("EnableCapabilityi not implemented yet")
	}
	void GLStateManager::DisableCapabilityi(GLenum, GLuint)
	{
		ST_THROW("DisableCapabilityi not implemented yet")
	}
	...
	void GLStateManager::StencilOpState(
		GLenum face, GLenum func, GLint ref, GLuint mask, GLenum sfail, GLenum dpfail, GLenum dppass)
	{
		if (face == GL_FRONT)
		{
			if (
	...
	void GLStateManager::StencilCompareMask(GLenum face, GLuint mask)
	{
		if (face == GL_FRONT)
		{
			...
			{
				glStencilFuncSeparate(face, mStencilOpState.frontOpState.compareOp, mStencilOpState.frontOpState.reference, mask);
				mStencilOpState.frontOpState.compareMask = mask;
			}
		}
		else if (face == GL_BACK)
		{
			...
			{
				glStencilFuncSeparate(face, mStencilOpState.backOpState.compareOp, mStencilOpState.backOpState.reference, mask);
				mStencilOpState.backOpState.compareMask = mask;
			}
		}
	}
	void GLStateManager::StencilReference(GLenum face, GLint ref)
	{
		if (face == GL_FRONT)
		{
			...
			{
				glStencilFuncSeparate(face, mStencilOpState.frontOpState.compareOp, ref, mStencilOpState.frontOpState.compareMask);
				mStencilOpState.frontOpState.reference = ref;
			}
		}
		else if (face == GL_BACK)
		{
			...
			{
				glStencilFuncSeparate(face, mStencilOpState.backOpState.compareOp, ref, mStencilOpState.backOpState.compareMask);
				mStencilOpState.backOpState.reference = ref;
			}
		}
	}
	...
	{
		if (
			mBlendState.blendColor[0] != color[0] &&
			mBlendState.blendColor[1] != color[1] &&
			mBlendState.blendColor[2] != color[2] &&
			mBlendState.blendColor[3] != color[3])
		{
