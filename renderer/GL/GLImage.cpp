...
namespace Shit
{
	...
	void GLImage::AddDestroySignalListener(Slot<void(GLImage const *)> const &slot) const
	{
		mDestroySignal.Connect(slot);
	}
	...
	void GLImage::Init()
	{
		if (mCreateInfo.mipLevels == 0)
			mCreateInfo.mipLevels = static_cast<uint32_t>(
				std::floor(
					std::log2((std::max)((std::max)(mCreateInfo.extent.width, mCreateInfo.extent.height),
		...
		if (!bool(mCreateInfo.flags & ImageCreateFlagBits::MUTABLE_FORMAT_BIT) &&
			bool(mCreateInfo.usageFlags & (ImageUsageFlagBits::COLOR_ATTACHMENT_BIT | ImageUsageFlagBits::DEPTH_STENCIL_ATTACHMENT_BIT)) &&
				...
				else
				{
					...
					case ImageType::TYPE_1D:
						glTexImage2D(
							...
							0,
							MapPixelFormat(mCreateInfo.format),
	...
	void GLImage::MapMemory(uint64_t offset, [[maybe_unused]] uint64_t size, void **ppData) const
	{
		if (mImageFlag == GLImageFlag::TEXTURE)
		{
			...
			glGetTexImage(target, 0, MapBaseFormat(mCreateInfo.format), Map(GetFormatDataType(mCreateInfo.format)), mTempData.data());
	...
	void GLImage::UnMapMemory() const
	{
	...
	void GLImage::InvalidateTexSubImage(int level, int xoffset, int yoffset, int zoffset, uint32_t width, uint32_t height, uint32_t depth) const
	{
		if (mImageFlag == GLImageFlag::TEXTURE)
	...
	void GLImage::GetImageSubresourceLayout([[maybe_unused]] const ImageSubresource &subresouce, [[maybe_unused]] SubresourceLayout &subresourceLayout) const
	{
		//
