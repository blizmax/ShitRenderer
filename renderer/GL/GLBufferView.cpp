...
namespace Shit
{
		...
		: BufferView(createInfo), mpStateManager(pStateManager)
	{
		...
		//glTexBufferRange(GL_TEXTURE_BUFFER, MapInternalFormat(mCreateInfo.format), buffer, static_cast<GLintptr>(mCreateInfo.offset), static_cast<GLsizeiptr>(mCreateInfo.range));
