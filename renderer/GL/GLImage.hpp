...
namespace Shit
{
	...
	public:
		...
		/**
		 * @brief if image is renderer buffer or multisample image or backbuffer, do not use this method
		 * 
		 * @param imageSubData 
		 */
		void UpdateSubData(
		...
		constexpr GLImageFlag GetImageFlag() const
		{
			return mImageFlag;
		}
		...
		void InvalidateTexSubImage(int level, int xoffset, int yoffset, int zoffset, uint32_t width, uint32_t height, uint32_t depth) const override;

		void GenerateMipmap(Filter filter, ImageLayout intialLayout, ImageLayout finalLayout) const override;

		void GetImageSubresourceLayout(const ImageSubresource &subresouce, SubresourceLayout &subresourceLayout) const override;
	...
	private:
		...
		GLImageFlag mImageFlag{GLImageFlag::TEXTURE}; //0 stand for texture,1 renderer buffer, 2 backbuffer
