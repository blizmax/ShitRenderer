...
namespace Shit
{
	...
	GLuint GLCommandBuffer::DummyFramebuffer::BindImage(
		GLStateManager *pManager,
		GLImage const *pImage,
		...
		uint32_t const *levels)
	{
		...
		if (!bound && _colorAttachment == pImage)
		{
			...
			{
				if (levels[i] == _colorAttachmentLevels[i])
					bound = true;
			}
		}
		if (bound)
		...
		pImage->AddDestroySignalListener(std::bind(&GLCommandBuffer::DummyFramebuffer::ImageDestroyListener, this, std::placeholders::_1));
		...
		if (bindPoint == GL_COLOR_ATTACHMENT0)
			glDrawBuffers(levelCount, attachpoints.data());
		{
			auto status = glCheckFramebufferStatus(GL_DRAW_FRAMEBUFFER);
			if (status != GL_FRAMEBUFFER_COMPLETE)
				ST_THROW("framebuffe error code:", status);
		}
	...
	GLCommandBuffer::GLCommandBuffer(
		GLStateManager *pStateManager, GLCommandPool *pCommandPool, CommandBufferLevel level)
	...
	size_t GLCommandBuffer::_BeginTransformFeedback(const void *params)
	{
		...
		return offsetof(BeginTransformFeedbackInfo, ppCounterBuffers) + (sizeof(ptrdiff_t) + sizeof(uint64_t)) * cmd->counterBufferCount;
	}
	...
	size_t GLCommandBuffer::_BindPipeline(const void *params)
	{
		...
		if (pPipeline)
		{
			...
			if (cmd->bindPoint == PipelineBindPoint::GRAPHICS)
			{
				...
				{
					mpStateManager->SetViewports(0,
												 pGraphicsPipelineCreateInfo->viewportState.viewportCount,
				...
				if (!dynamicFlags[static_cast<size_t>(DynamicState::SCISSOR)])
				{
					...
					mpStateManager->SetScissors(0,
												pGraphicsPipelineCreateInfo->viewportState.scissorCount,
				...
				if (pGraphicsPipelineCreateInfo->depthStencilState.depthTestEnable)
				{
					...
					// mpCurFramebuffer->SetDepthMask(mCurSubpass, pGraphicsPipelineCreateInfo->depthStencilState.depthWriteEnable);
				}
				else
				{
				...
				if (pGraphicsPipelineCreateInfo->depthStencilState.stencilTestEnable)
				{
					...
					// mpCurFramebuffer->SetStencilMask(mCurSubpass, pGraphicsPipelineCreateInfo->depthStencilState.front.writeMask);
					// mpCurFramebuffer->SetStencilBackMask(mCurSubpass, pGraphicsPipelineCreateInfo->depthStencilState.back.writeMask);
				}
				else
				{
				...
				for (uint32_t i = 0; i < pGraphicsPipelineCreateInfo->colorBlendState.attachmentCount; ++i)
				{
					...
					// mpCurFramebuffer->SetColorMask(mCurSubpass, i, (GLuint)e.colorWriteMask);
				}
	...
	size_t GLCommandBuffer::_BindDescriptorSets(const void *params)
	{
		...
		return offsetof(BindDescriptorSetsInfo, ppDescriptorSets) +
			   sizeof(ptrdiff_t) * cmd->descriptorSetCount +
	...
	size_t GLCommandBuffer::_BindTransformFeedBackBuffers(const void *params)
	{
		...
		return offsetof(BindTransformFeedbackBuffersInfo, ppBuffers) + (sizeof(ptrdiff_t) + sizeof(uint64_t) * 2) * cmd->bindingCount;
	}
	size_t GLCommandBuffer::_BlitImage(const void *params)
	{
		...
		auto srcFBO = mDummyFramebuffer.BindImage(
			mpStateManager, static_cast<GLImage const *>(cmd->pSrcImage), ImageAspectFlagBits::COLOR_BIT, cmd->regionCount, srcLevels.data());
		auto dstFBO = mDummyFramebuffer2.BindImage(
			mpStateManager, static_cast<GLImage const *>(cmd->pDstImage), ImageAspectFlagBits::COLOR_BIT, cmd->regionCount, dstLevels.data());
		...
		for (uint32_t i = 0; i < cmd->regionCount; ++i)
		{
			...
			GLenum mask = 0;
			if (bool(e.srcSubresource.aspectMask & ImageAspectFlagBits::COLOR_BIT))
				mask |= GL_COLOR_BUFFER_BIT;
			if (bool(e.srcSubresource.aspectMask & ImageAspectFlagBits::DEPTH_BIT))
				mask |= GL_DEPTH_BUFFER_BIT;
			if (bool(e.srcSubresource.aspectMask & ImageAspectFlagBits::STENCIL_BIT))
	...
	size_t GLCommandBuffer::_ClearColorImage(const void *params)
	{
		...
		auto fbo = mDummyFramebuffer.BindImage(
			mpStateManager, pImage, ImageAspectFlagBits::COLOR_BIT, (uint32_t)levels.size(), levels.data());
	...
	size_t GLCommandBuffer::_CopyBuffer(const void *params)
	{
		...
		for (uint32_t i = 0; i < cmd->regionCount; ++i)
		{
			glCopyBufferSubData(
				...
				static_cast<GLintptr>(regions[i].srcOffset),
				static_cast<GLintptr>(regions[i].dstOffset),
...
// opengl 4.2
	...
	size_t GLCommandBuffer::_FillBuffer(const void *params)
	{
		...
		auto size = cmd->size;
		if (size == ST_WHOLE_SIZE)
			size = buffer->GetCreateInfoPtr()->size - cmd->offset;
		std::vector<uint32_t> data(size / 4, cmd->data);
		glClearBufferSubData(GL_ARRAY_BUFFER, GL_R32UI, cmd->offset, size, GL_RED_INTEGER, GL_UNSIGNED_INT, (const void *)data.data());
		return sizeof(*cmd);
	}
	...
	size_t GLCommandBuffer::_PipelineBarrier(const void *params)
	{
		...
		return offsetof(PipelineBarrierInfo, pMemoryBarriers) +
			   +sizeof(uint32_t) * 2 +
	...
	size_t GLCommandBuffer::_PushConstants(const void *params)
	{
		...
		dynamic_cast<GLPipelineLayout const *>(cmd->pPipelineLayout)->PushConstant(cmd->stageFlags, cmd->offset, cmd->size, &cmd->pValues);
	...
	size_t GLCommandBuffer::_ResolveImage(const void *params)
	{
		...
		for (uint32_t i = 0; i < cmd->regionCount; ++i)
		{
			...
			glBlitFramebuffer(
				e.srcOffset.x,
				e.srcOffset.y,
				e.extent.width - e.srcOffset.x,
				e.extent.height - e.srcOffset.y,
				e.dstOffset.x,
				e.dstOffset.y,
				e.extent.width - e.dstOffset.x,
				e.extent.height - e.dstOffset.y,
				mask,
				GL_LINEAR);
		}
		mpStateManager->PopReadFramebuffer();
	...
	size_t GLCommandBuffer::_ExecuteCommands(const void *params)
	{
		...
		auto pCommandBuffers = const_cast<CommandBuffer **>(reinterpret_cast<CommandBuffer const *const *>(&cmd->pCommandBuffers));
	...
	size_t GLCommandBuffer::_SetDepthBias(const void *params)
	{
		...
		mpStateManager->PolygonOffSetClamp(
			cmd->depthBiasSlopFactor, cmd->depthBiasConstantFactor, cmd->depthBiasClamp);
		return sizeof(*cmd);
	}
	...
	size_t GLCommandBuffer::_SetScissor(const void *params)
	{
		...
		mpStateManager->SetScissors(cmd->firstScissor, cmd->scissorCount, reinterpret_cast<const int32_t *>(&cmd->pScissors));
	...
	size_t GLCommandBuffer::_SetViewport(const void *params)
	{
		...
		mpStateManager->SetViewports(cmd->firstViewport, cmd->viewportCount, reinterpret_cast<const float *>(&cmd->pViewports));
	...
	void GLCommandBuffer::ExecuteCommands(const ExecuteCommandsInfo &info)
	{
		auto p = AllocateCommand<ExecuteCommandsInfo>(
			GLCommandCode::EXECUTE_COMMANDS, offsetof(ExecuteCommandsInfo, pCommandBuffers) + sizeof(ptrdiff_t) * info.count);
		p->count = info.count;
		memcpy(&p->pCommandBuffers, info.pCommandBuffers, sizeof(ptrdiff_t) * info.count);
	}
	void GLCommandBuffer::Reset(CommandBufferResetFlatBits)
	{
		mBuffer.clear();
	}
	void GLCommandBuffer::Begin(const CommandBufferBeginInfo &beginInfo)
	{
		...
		if (beginInfo.inheritanceInfo.pRenderPass)
		{
			...
			mpCurFramebuffer = const_cast<GLFramebuffer *>(static_cast<GLFramebuffer const *>(beginInfo.inheritanceInfo.pFramebuffer));
		}
	}
	...
	void GLCommandBuffer::BeginRenderPass(const BeginRenderPassInfo &beginInfo)
	{
		...
		auto p = AllocateCommand<BeginRenderPassInfo>(
			GLCommandCode::BEGIN_RENDERPASS, offsetof(BeginRenderPassInfo, clearValueCount));
		memcpy(p, &beginInfo, offsetof(BeginRenderPassInfo, clearValueCount));
	}
	void GLCommandBuffer::EndRenderPass()
	{
		AllocateCommand<void>(GLCommandCode::END_RENDERPASS);
	}
	...
	void GLCommandBuffer::BlitImage(const BlitImageInfo &blitInfo)
	{
		auto p = AllocateCommand<BlitImageInfo>(
			GLCommandCode::BLIT_IMAGE,
			offsetof(BlitImageInfo, pRegions) + sizeof(ImageBlit) * blitInfo.regionCount);
	...
	{
		auto p = AllocateCommand<BindDescriptorSetsInfo>(
			GLCommandCode::BIND_DESCRIPTORSETS,
			offsetof(BindDescriptorSetsInfo, ppDescriptorSets) + sizeof(ptrdiff_t) * info.descriptorSetCount +
	...
	void GLCommandBuffer::Draw(const DrawIndexedIndirectCommand &info)
	{
		DrawIndexed(info);
	}
	...
	void GLCommandBuffer::DrawIndirect(const DrawIndirectCountInfo &info)
	{
		DrawIndirectCount(info);
	}
	void GLCommandBuffer::DrawIndirectCount(const DrawIndirectCountInfo &info)
	{
		memcpy(AllocateCommand<DrawIndirectCountInfo>(GLCommandCode::DRAW_INDIRECT_COUNT), &info, sizeof(DrawIndirectCountInfo));
	}
	void GLCommandBuffer::DrawIndexed(const DrawIndexedIndirectCommand &info)
	{
		memcpy(AllocateCommand<DrawIndexedIndirectCommand>(GLCommandCode::DRAW_INDEXED), &info, sizeof(DrawIndexedIndirectCommand));
	}
	void GLCommandBuffer::DrawIndexedIndirect(const DrawIndirectInfo &info)
	{
		...
		//	}
		//	mpStateManager->BindBuffer(GL_COPY_WRITE_BUFFER, stageBuffer);
		//	mpStateManager->BindBuffer(GL_COPY_READ_BUFFER, static_cast<GLBuffer const *>(info.pBuffer)->GetHandle());
		...
		//	mpStateManager->BindBuffer(GL_COPY_WRITE_BUFFER, static_cast<GLBuffer const *>(info.pBuffer)->GetHandle());
		//	glCopyBufferSubData(GL_COPY_READ_BUFFER, GL_COPY_WRITE_BUFFER, 0, 0, sizeof(DrawIndexedIndirectCommand));
	...
	void GLCommandBuffer::DrawIndexedIndirect(const DrawIndirectCountInfo &info)
	{
		DrawIndexedIndirectCount(info);
	}
	void GLCommandBuffer::DrawIndexedIndirectCount(const DrawIndirectCountInfo &info)
	{
		memcpy(AllocateCommand<DrawIndirectCountInfo>(GLCommandCode::DRAW_INDEXED_INDIRECT_COUNT), &info, sizeof(DrawIndirectCountInfo));
	}
	void GLCommandBuffer::ClearColorImage(const ClearColorImageInfo &info)
	{
		auto p = AllocateCommand<ClearColorImageInfo>(
			GLCommandCode::CLEAR_COLOR_IMAGE, offsetof(ClearColorImageInfo, pRanges) + sizeof(ImageSubresourceRange) * info.rangeCount);
	...
	void GLCommandBuffer::ClearDepthStencilImage(const ClearDepthStencilImageInfo &info)
	{
		auto p = AllocateCommand<ClearDepthStencilImageInfo>(
			GLCommandCode::CLEAR_DEPTH_STENCIL_IMAGE, offsetof(ClearDepthStencilImageInfo, pRanges) + sizeof(ImageSubresourceRange) * info.rangeCount);
	...
	void GLCommandBuffer::PipelineBarrier(const PipelineBarrierInfo &info)
	{
		auto p = AllocateCommand<PipelineBarrierInfo>(
			GLCommandCode::PIPELINE_BARRIER,
			offsetof(PipelineBarrierInfo, pMemoryBarriers) + sizeof(uint32_t) * 2 +
	...
	void GLCommandBuffer::PushConstants(const PushConstantInfo &info)
	{
		auto p = AllocateCommand<PushConstantInfo>(
			GLCommandCode::PUSH_CONSTANTS, offsetof(PushConstantInfo, pValues) + info.size);
	...
	void GLCommandBuffer::BindTransformFeedbackBuffers(const BindTransformFeedbackBuffersInfo &info)
	{
		auto p = AllocateCommand<BindTransformFeedbackBuffersInfo>(
			...
			offsetof(BindTransformFeedbackBuffersInfo, ppBuffers) + (sizeof(ptrdiff_t) + sizeof(uint64_t) * 2) * info.bindingCount);
	...
	void GLCommandBuffer::BeginTransformFeedback(const BeginTransformFeedbackInfo &info)
	{
		auto p = AllocateCommand<BeginTransformFeedbackInfo>(
			GLCommandCode::BEGIN_TRANSFORMFEEDBACK,
			offsetof(BeginTransformFeedbackInfo, ppCounterBuffers) + (sizeof(ptrdiff_t) + sizeof(uint64_t)) * info.counterBufferCount);
	...
	void GLCommandBuffer::EndTransformFeedback(const EndTransformFeedbackInfo &info)
	{
		auto p = AllocateCommand<EndTransformFeedbackInfo>(
			GLCommandCode::END_TRANSFORMFEEDBACK,
			offsetof(EndTransformFeedbackInfo, ppCounterBuffers) + (sizeof(ptrdiff_t) + sizeof(uint64_t)) * info.counterBufferCount);
	...
	{
		auto p = AllocateCommand<SetScissorInfo>(
			GLCommandCode::SET_SCISSOR, offsetof(SetScissorInfo, pScissors) + sizeof(Rect2D) * info.scissorCount);
	...
	void GLCommandBuffer::ResolveImage(const ResolveImageInfo &info)
	{
		auto p = AllocateCommand<ResolveImageInfo>(
			GLCommandCode::RESOLVE_IMAGE,
			offsetof(ResolveImageInfo, pRegions) + sizeof(ImageResolve) * info.regionCount);
