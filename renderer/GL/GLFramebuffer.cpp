...
namespace Shit
{
	...
	void GLFramebuffer::FBOState::PopDrawBuffer()
	{
		PopDrawBuffers();
	}
	...
	void GLFramebuffer::Resolve(uint32_t fboIndex, Filter filter) const
	{
		if (mFBOs.at(fboIndex).second)
		{
			...
			for (size_t i = 0, l = renderFBO->colorAttachments.size(); i < l; ++i)
			{
				...
				glBlitFramebuffer(0, 0, mCreateInfo.extent.width, mCreateInfo.extent.height,
	...
	void GLFramebuffer::CreateFBO(uint32_t fboIndex, bool isRenderFBO)
	{
		...
		if (imageFlag == GLImageFlag::TEXTURE || imageFlag == GLImageFlag::RENDERBUFFER)
		{
			...
			for (auto aa : fbo->colorAttachments)
				drawBuffers.emplace_back(aa.attachPoint);
			...
			if (e.pDepthStencilAttachment && isRenderFBO)
			{
				...
				pImageView = static_cast<const GLImageView *>(mCreateInfo.pAttachments[e.pDepthStencilAttachment->attachment]);
				pImage = static_cast<GLImage const *>(pImageView->GetCreateInfoPtr()->pImage);
				imageFlag = pImage->GetImageFlag();
				switch (imageFlag)
				{
		...
		else
		{
			...
			{
				pImageView = static_cast<const GLImageView *>(mCreateInfo.pAttachments[attachments[j].attachment]);
				fbo->colorAttachments[j].attachPoint = GetFBOBindPoint(static_cast<const GLImage *>(pImageView->GetCreateInfoPtr()->pImage)->GetImageFlag());
			}
			mpStateManager->BindDrawFramebuffer(fbo->handle);
			std::vector<GLenum> drawBuffers;
			for (auto aa : fbo->colorAttachments)
			...
			if (imageFlag == GLImageFlag::TEXTURE)
			{
				if (pImage->GetCreateInfoPtr()->samples == Shit::SampleCountFlagBits::BIT_1)
				{
					glFramebufferTextureMultiviewOVR(
						GL_DRAW_FRAMEBUFFER, bindpoint, imageView->GetHandle(), 0, 0, imageView->GetCreateInfoPtr()->subresourceRange.layerCount);
				}
				else
				{
					glFramebufferTextureMultisampleMultiviewOVR(
						GL_DRAW_FRAMEBUFFER, bindpoint, imageView->GetHandle(), 0,
						(GLsizei)(pImage->GetCreateInfoPtr()->samples), 0, imageView->GetCreateInfoPtr()->subresourceRange.layerCount);
				}
			}
			else
			{
		...
		: Framebuffer(createInfo), mpStateManager(pStateManager)
	{
		if (mCreateInfo.pRenderPass->GetCreateInfoPtr()->pMultiviewCreateInfo)
		{
			...
			if (mCreateInfo.pRenderPass->GetCreateInfoPtr()->pAttachments[0].samples != Shit::SampleCountFlagBits::BIT_1)
			{
				...
				{
					ST_LOG("GL_OVR_multiview_multisampled_render_to_texture is not supported, cannot use multiview ")
				}
			}
		}
	...
	void GLFramebuffer::ClearBuffer(uint32_t fboIndex) const
	{
		...
		for (uint32_t i = 0; i < subpassDesc.colorAttachmentCount; ++i)
		{
			...
			if (attachments[e.attachment].loadOp == AttachmentLoadOp::CLEAR)
			{
				...
				auto v = static_cast<GLRenderPass const *>(mCreateInfo.pRenderPass)->GetAttachmentClearValuePtr(e.attachment);
				...
				if (formatNumeric == FormatNumeric::SINT)
					...
					glClearBufferuiv(GL_COLOR, i, reinterpret_cast<const uint32_t *>(std::get_if<ClearColorValueUint32>(v)));
				else
		...
		if (subpassDesc.pDepthStencilAttachment)
		{
			...
			int target = 0;
			if (attachments[attachmentIndex].loadOp == AttachmentLoadOp::CLEAR)
				target |= 1;
			if (attachments[attachmentIndex].stencilLoadOp == AttachmentLoadOp::CLEAR)
				target |= 2;
			if (target > 0)
			{
				auto clearValue = static_cast<GLRenderPass const *>(mCreateInfo.pRenderPass)->GetAttachmentClearValuePtr(attachmentIndex);
				int flag = 0;
				...
				if (flag == 1)
					...
					glClearBufferfi(GL_DEPTH_STENCIL, 0,
									std::get_if<ClearDepthStencilValue>(clearValue)->depth,
