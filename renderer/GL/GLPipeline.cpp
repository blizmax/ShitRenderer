...
namespace Shit
{
	...
	void GLPipelineLayout::PushConstant(
		ShaderStageFlagBits stageFlags, uint32_t offset, uint32_t size, void const *pValues) const
	{
		for (uint32_t i = 0; i < mCreateInfo.pushConstantRangeCount; ++i)
		{
	...
	GLVAO::GLVAO(GLGraphicsPipeline *pipeline) : mPipeline(pipeline)
	{
		...
		if (mHandle == 0)
			ST_THROW("failed to create vertex array object")
		Bind();
	...
	{
		if (mVertexAttributes[first].buffer != buffer ||
			mVertexAttributes[first].offset != offset ||
	...
	void GLVAO::BindVertexBuffers(GLuint first, GLsizei count,
								  std::span<const GLuint> buffers,
								  std::span<const GLintptr> offsets,
								  std::span<const GLsizei> strides)
	{
	...
	GLenum GLVAO::GetIndexType() const
	{
		return mIndexBufferState.type;
	}
	//=====================================
	GLPipeline::GLPipeline(GLStateManager *pStateManager)
	...
	GLuint GLPipeline::CreateShader(const PipelineShaderStageCreateInfo &shaderStageCreateInfo)
	{
		...
		if (shader)
		{
			...
			if (static_cast<bool>((shaderCreateInfoPtr->shadingLanguage & ShadingLanguage::TypeBitmask) == ShadingLanguage::GLSL))
			{
			...
			else if (static_cast<bool>((shaderCreateInfoPtr->shadingLanguage & ShadingLanguage::TypeBitmask) == ShadingLanguage::SPIRV))
			{
				glShaderBinary(1, &shader, GL_SHADER_BINARY_FORMAT_SPIR_V,
							   shaderCreateInfoPtr->code,
	...
	void GLPipeline::InitializePipeline()
	{
		...
		//for (uint32_t i = 0; i < mCreateInfo.pLayout->GetCreateInfoPtr()->pushConstantRangeCount; ++i)
		//{
		//	auto &&pcRange = mCreateInfo.pLayout->GetCreateInfoPtr()->pPushConstantRanges[i];
		//	for (auto &&e : mProgramsInfo)
		//	{
		//		if (bool(e.stage & pcRange.stageFlags))
		//		{
		//			e.pushConstantBufferBinding = glGetProgramResourceLocation(e.program, GL_UNIFORM, pcRange.name);
		//		}
		//	};
		//}
	...
	void GLGraphicsPipeline::Initialize()
	{
		...
		ST_LOG("GL_ACTIVE_RESOURCES");
		ST_LOG_VAR(activeResouces);
		GLint a;
		...
		ST_LOG("GL_MAX_NUM_ACTIVE_VARIABLES");
		ST_LOG_VAR(a);
...
*/
	...
	// gl 4.4
	void GLGraphicsPipeline::BindVertexBuffers(GLuint first, GLsizei count,
											   std::span<const GLuint> buffers,
											   std::span<const GLintptr> offsets,
											   std::span<const GLsizei> strides)
	{
	...
	void GLGraphicsPipeline::BindVertexBuffers(BindVertexBuffersInfo const &info)
	{
		...
		// std::vector<GLintptr> offsets(info.pOffsets, info.pOffsets + info.bindingCount);
		...
		std::ranges::transform(info.ppBuffers,
							   info.ppBuffers + info.bindingCount,
							   std::back_inserter(vertexBuffers),
							   [](auto p)
							   {
								   return p ? static_cast<GLBuffer const *>(p)->GetHandle() : GL_NONE;
							   });
		std::ranges::transform(info.pOffsets,
							   info.pOffsets + info.bindingCount,
							   std::back_inserter(offsets),
							   [](auto p)
							   {
								   return static_cast<GLintptr>(p);
							   });
		...
		for (; r < bindingCount; ++r)
		{
			...
			if (bool(vertexBuffers[r]) != bool(vertexBuffers[r - 1]))
			{
				...
				BindVertexBuffers(l, r - l,
								  std::span{&vertexBuffers[l], r - l},
								  std::span{&offsets[l], r - l},
	...
	GLenum GLGraphicsPipeline::GetIndexType() const
	{
		return mVAO->GetIndexType();
	}
	//======================================
