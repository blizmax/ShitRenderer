...
namespace Shit
{
	...
	{
		std::variant<PhysicalDevice, Window *> physicalDevice; // use shitwindow for vulkan
	};
	...
	struct SwapchainCreateInfo
	{
		...
		Window const *pWindow; //!< not used by opengl
	};
	...
	struct VertexBindingDescription
	{
		uint32_t binding; // index in given buffers
		uint32_t stride;
		uint32_t divisor; // attributes advance once per divior instances,when 0, advance per vertex, vulkan only support 0 and 1 currently
	};
	...
	struct CommandBufferInheritanceInfo
	{
		...
		uint32_t subpass;
		Framebuffer const *pFramebuffer; // optional, when use secondary cmdbuffer, this can be set
	};
	struct CommandBufferBeginInfo
	{
		CommandBufferUsageFlagBits usage;
		CommandBufferInheritanceInfo inheritanceInfo; // only used when commandBuffer is a secondary buffer
	};
	...
	using ClearValue = std::variant<ClearColorValueFloat, ClearColorValueInt32, ClearColorValueUint32, ClearDepthStencilValue>;
	...
	struct PushConstantRange
	{
		...
		uint32_t binding;				// only needed in opengl, use non opaque uniform block as push constant
		uint32_t offset;
		uint32_t size;
	};
	...
	{
		uint32_t attachment; // canbe ST_ATTACHMENT_UNUSED, index of attachment description
		ImageLayout layout;
	};
	...
	struct FramebufferCreateInfo
	{
		RenderPass const *pRenderPass;
		uint32_t attachmentCount;
		ImageView const *const *pAttachments; // the same order as attachment descriptions in renderpassCreateInfo
		Extent2D extent;
		uint32_t layers; // if use multiview, must be 1
		//if attachmentcount is not 0, if pAttachmentImageInfos is not nullptr, it means using imageless framebuffer,
		FramebufferAttachmentImageInfo const *pAttachmentImageInfos;
	};
	...
	{
		uint64_t timeout;
		Semaphore const *pSemaphore; // must be unsigal, will be signaled after operation
		Fence const *pFence;
	};
	struct SubmitInfo
	{
		uint32_t waitSemaphoreCount;
		Semaphore const *const *pWaitSemaphores; //! wait pipeline stage is color attachment output, will be unsignaled
	...
	{
		Buffer const *pBuffer;
		uint64_t offset; //TODO: attention!!!, gl version donot support index buffer offset, offset must be 0
		IndexType indexType;
	};
