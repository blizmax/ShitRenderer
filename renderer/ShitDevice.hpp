...
namespace Shit
{
	...
	public:
		...
		virtual std::optional<QueueFamilyProperty> GetPresentQueueFamilyProperty(Window const *) const
		{
			// for opengl
		...
		virtual std::optional<QueueFamilyProperty> GetQueueFamilyProperty(QueueFlagBits) const
		{
			// for opengl
			return std::optional<QueueFamilyProperty>{
				{Shit::QueueFlagBits::GRAPHICS_BIT |
		...
		virtual void GetWindowPixelFormats(const Window *pWindow, std::vector<WindowPixelFormat> &formats) const = 0;
		virtual void GetPresentModes(const Window *pWindow, std::vector<PresentMode> &presentModes) const = 0;

		virtual Format GetSuitableImageFormat(std::span<const Format> candidates, ImageTiling, FormatFeatureFlagBits) const
		{
			return candidates[0];
		}
		...
		virtual Image *Create(const ImageCreateInfo &createInfo,
		...
		virtual void UpdateDescriptorSets(
			...
			const CopyDescriptorSet *descriptorCopies) = 0;
