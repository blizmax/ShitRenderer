...
namespace Shit
{
	...
	protected:
		...
		DescriptorSetLayout(const DescriptorSetLayoutCreateInfo &createInfo)
		{
			...
			{
				mCreateInfo.pDescriptorSetLayoutBindings = new DescriptorSetLayoutBinding[mCreateInfo.descriptorSetLayoutBindingCount];
				memcpy((void *)mCreateInfo.pDescriptorSetLayoutBindings,
					   createInfo.pDescriptorSetLayoutBindings,
			...
			for (uint32_t i = 0; i < mCreateInfo.descriptorSetLayoutBindingCount; ++i)
			{
				...
				if (e->immutableSamplerCount)
				{
					...
					memcpy((void *)e->pImmutableSamplers,
						   createInfo.pDescriptorSetLayoutBindings[i].pImmutableSamplers,
	...
	public:
		DescriptorPool(const DescriptorPoolCreateInfo &createInfo) : mCreateInfo(createInfo)
		{
			...
			memcpy((void *)mCreateInfo.poolSizes, createInfo.poolSizes, sizeof(DescriptorPoolSize) * mCreateInfo.poolSizeCount);
		}
		virtual ~DescriptorPool()
		{
			delete[] mCreateInfo.poolSizes;
		}
		 ...
		 */
		virtual void Allocate(const DescriptorSetAllocateInfo &allocateInfo, std::vector<DescriptorSet *> &descriptorSets) = 0;
		...
		{
			while (count-- > 0)
				mDescriptorSets.erase(const_cast<DescriptorSet *>(ppDescriptorSets[count]));
		}
	};
}