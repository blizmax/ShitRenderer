...
namespace Shit
{
	template <typename Fn,
			  std::enable_if_t<std::is_function_v<Fn>, bool> = true>
	...
	public:
		...
		template <typename... Args,
				  std::enable_if_t<std::is_invocable_v<Fn, Args...>, bool> = true>
	...
	public:
		...
		locked_container_type lock() const
		{
			...
			for (auto &&e : _trackedObjects)
			{
				ret.emplace_back(std::visit(
					overloaded(
						[](const std::weak_ptr<void> &arg)
						{
							if (arg.expired())
								throw std::runtime_error("expired slot");
							return arg.lock();
						}),
					e));
			}
	...
	protected:
		void track_signal(const SignalBase &signal)
		{
			_trackedObjects.emplace_back(signal.lock_pimpl());
		}
	};
	...
	{
	public:
		using function_type = std::function<_Signature>;
		...
		{
			for (auto &&e : v.tracked_objects())
				_trackedObjects.emplace_back(e);
			return *this;
		}
	...
	public:
		using result_type = _ConnectionBody::slot_type::result_type;

		SlotCallIterator(_ConnectionBodyIter it, std::shared_ptr<std::tuple<_Args...>> args)
			: _iter(it), _args(args)
		{
		}
		result_type operator*()
		{
			return invokeFunc(std::make_index_sequence<sizeof...(_Args)>{});
		}
		bool operator==(const SlotCallIterator &other)
		{
	...
	public:
		...
		bool SlotExpired() const override
		{
	...
	public:
		...
		{
			if (!_weakConnectionBody.expired())
				_weakConnectionBody.lock()->Disconnect();
		}
		bool Connected() const noexcept
		{
			if (!_weakConnectionBody.expired())
				_weakConnectionBody.lock()->Connected();
		}
	...
	template <typename _Signature,
			  typename _Combiner = last_value<typename std::function<_Signature>::result_type>,
			  typename _Group = int,
			  std::enable_if_t<std::is_function_v<_Signature>, bool> = true>
	...
	public:
		...
		using result_type = combiner_type::result_type;
		...
		Connection Connect(group_type group, const slot_type &slot)
		{
			const std::lock_guard<std::mutex> lock(*_mutex);
		...
		{
			return std::find_if(_connections.cbegin(), _connections.cend(), [&slot](auto &&e)
								{ return *e->GetSlot() == slot; }) != _connections.cend();
		}
		void Disconnect(const slot_type &slot)
		{
			auto it = std::find_if(_connections.cbegin(), _connections.cend(), [&slot](auto &&e)
								   { return *e->GetSlot() == slot; });
		...
		constexpr size_t ConnectionNum() const
		{
		...
		void ClearExpiredConnections() const
		{
			while (true)
			{
				auto it = std::ranges::find_if(
					_connections, [](auto &&e)
					{auto a=e->GetSlot();return !a||e->SlotExpired(); });
		...
		result_type operator()(Args &&...args) const
		{
			...
			return _combiner(
				SlotCallIterator<connection_body_type, connection_container_iterator_type, Args...>(_connections.begin(), a),
	...
	private:
		...
		std::shared_ptr<void> lock_pimpl() const override
		{
			return _handle;
		}
	};
