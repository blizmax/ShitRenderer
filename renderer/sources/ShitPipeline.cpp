...
namespace Shit
{
	...
	GraphicsPipeline::GraphicsPipeline(const GraphicsPipelineCreateInfo &createInfo) : mCreateInfo(createInfo)
	{
		mCreateInfo.stages = new PipelineShaderStageCreateInfo[mCreateInfo.stageCount];
		memcpy((void *)mCreateInfo.stages, createInfo.stages, sizeof(PipelineShaderStageCreateInfo) * mCreateInfo.stageCount);

		for (uint32_t i = 0; i < mCreateInfo.stageCount; ++i)
		{
			auto &&e = const_cast<SpecializationInfo *>(&mCreateInfo.stages[i].specializationInfo);
			auto &&s = createInfo.stages[i].specializationInfo;
			if (e->constantValueCount)
			{
				e->constantIds = new uint32_t[e->constantValueCount];
				memcpy((void *)e->constantIds, s.constantIds, sizeof(uint32_t) * e->constantValueCount);
				e->constantValues = new uint32_t[e->constantValueCount];
				memcpy((void *)e->constantValues, s.constantValues, sizeof(uint32_t) * e->constantValueCount);
			}
		}

		if (mCreateInfo.vertexInputState.vertexBindingDescriptionCount)
		{
			mCreateInfo.vertexInputState.vertexBindingDescriptions = new VertexBindingDescription[mCreateInfo.vertexInputState.vertexBindingDescriptionCount];
			memcpy((void *)mCreateInfo.vertexInputState.vertexBindingDescriptions,
				   createInfo.vertexInputState.vertexBindingDescriptions,
				   sizeof(VertexBindingDescription) * mCreateInfo.vertexInputState.vertexBindingDescriptionCount);
		}
		if (mCreateInfo.vertexInputState.vertexAttributeDescriptionCount)
		{
			mCreateInfo.vertexInputState.vertexAttributeDescriptions = new VertexAttributeDescription[mCreateInfo.vertexInputState.vertexAttributeDescriptionCount];
			memcpy((void *)mCreateInfo.vertexInputState.vertexAttributeDescriptions,
				   ...
				   sizeof(VertexAttributeDescription) * mCreateInfo.vertexInputState.vertexAttributeDescriptionCount);
		}

		//viewport
		if (mCreateInfo.viewportState.viewportCount)
		{
			mCreateInfo.viewportState.viewports = new Viewport[mCreateInfo.viewportState.viewportCount];
		...
		{
			mCreateInfo.viewportState.scissors = new Rect2D[mCreateInfo.viewportState.scissorCount];
		...
		{
			//color blend
			mCreateInfo.colorBlendState.attachments = new PipelineColorBlendAttachmentState[mCreateInfo.colorBlendState.attachmentCount];
			memcpy((void *)mCreateInfo.colorBlendState.attachments,
				   createInfo.colorBlendState.attachments,
				   sizeof(PipelineColorBlendAttachmentState) * mCreateInfo.colorBlendState.attachmentCount);
		}

		if (mCreateInfo.dynamicState.dynamicStateCount)
		{
			//dynamic state
			mCreateInfo.dynamicState.dynamicStates = new DynamicState[mCreateInfo.dynamicState.dynamicStateCount];
	...
	GraphicsPipeline::~GraphicsPipeline()
	{
		...
		if (mCreateInfo.stageCount)
			delete[] mCreateInfo.stages;
		if (mCreateInfo.vertexInputState.vertexBindingDescriptionCount)
	...
	{
		auto &&e = const_cast<SpecializationInfo *>(&mCreateInfo.stage.specializationInfo);
