...
namespace Shit
{
	...
	void DeleteRenderSystem(RenderSystem *pRenderSystem)
	{
		...
		if (!f)
			ST_THROW("failed to find rendersystem delele function");
		f(pRenderSystem);
	}
	...
	RenderSystem *LoadRenderSystem(const RenderSystemCreateInfo &createInfo)
	{
		...
		if (!f)
			ST_THROW("failed to find rendersystem load function");
		InitEvent();
		...
		//auto ret = f(createInfo);
		//return ret;
	}

	Window *RenderSystem::CreateRenderWindow(const WindowCreateInfo &createInfo)
	{
#ifdef _WIN32
