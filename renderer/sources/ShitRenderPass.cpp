...
namespace Shit
{
	RenderPass::RenderPass(const RenderPassCreateInfo &createInfo)
	{
		...
		{
			mCreateInfo.pAttachments = new AttachmentDescription[mCreateInfo.attachmentCount];
			memcpy((void *)mCreateInfo.pAttachments, createInfo.pAttachments, sizeof(AttachmentDescription) * mCreateInfo.attachmentCount);
		}
		if (mCreateInfo.dependencyCount)
		{
			mCreateInfo.pSubpassDependencies = new SubpassDependency[mCreateInfo.dependencyCount];
			memcpy((void *)mCreateInfo.pSubpassDependencies, createInfo.pSubpassDependencies, sizeof(SubpassDependency) * mCreateInfo.dependencyCount);
		}
		...
		for (uint32_t i = 0; i < mCreateInfo.subpassCount; ++i)
		{
			...
			// input pAttachments
			if ((tempCount = ppSubpasses[i].inputAttachmentCount = src_pSubpasses.inputAttachmentCount) != 0)
			{
				ppSubpasses[i].pInputAttachments = new AttachmentReference[tempCount];
				memcpy((void *)ppSubpasses[i].pInputAttachments, src_pSubpasses.pInputAttachments, sizeof(AttachmentReference) * tempCount);
			}

			// color attachments
			if ((tempCount = ppSubpasses[i].colorAttachmentCount = src_pSubpasses.colorAttachmentCount) != 0)
			{
				ppSubpasses[i].pColorAttachments = new AttachmentReference[tempCount];
				memcpy((void *)ppSubpasses[i].pColorAttachments, src_pSubpasses.pColorAttachments, sizeof(AttachmentReference) * tempCount);
			}
			...
			if ((src_pSubpasses.pResolveAttachments) != 0)
			{
				...
				memcpy((void *)ppSubpasses[i].pResolveAttachments, src_pSubpasses.pResolveAttachments, sizeof(AttachmentReference) * tempCount);
			}
			// depth attachments
			if (src_pSubpasses.pDepthStencilAttachment)
				ppSubpasses[i].pDepthStencilAttachment = new AttachmentReference(*src_pSubpasses.pDepthStencilAttachment);
		}
		if (createInfo.pMultiviewCreateInfo)
		{
			auto a = new RenderPassMultiViewCreateInfo{};

			if ((a->subpassCount = createInfo.pMultiviewCreateInfo->subpassCount) != 0)
			{
				a->pViewMasks = new uint32_t[a->subpassCount];
				memcpy((void *)a->pViewMasks, createInfo.pMultiviewCreateInfo->pViewMasks, sizeof(uint32_t) * a->subpassCount);
			};
			if ((a->dependencyCount = createInfo.pMultiviewCreateInfo->dependencyCount) != 0)
			{
				a->pViewOffsets = new int32_t[a->dependencyCount];
				memcpy((void *)a->pViewOffsets, createInfo.pMultiviewCreateInfo->pViewOffsets, sizeof(uint32_t) * a->dependencyCount);
			}
			if ((a->correlationMaskCount = createInfo.pMultiviewCreateInfo->correlationMaskCount) != 0)
			{
				a->pCorrelationMasks = new uint32_t[a->correlationMaskCount];
				memcpy((void *)a->pCorrelationMasks, createInfo.pMultiviewCreateInfo->pCorrelationMasks, sizeof(uint32_t) * a->correlationMaskCount);
			}
