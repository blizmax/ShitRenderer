...
namespace Shit
{
	static uint32_t keyCodeArrayWin32[]{
		VK_LBUTTON, //        0x01
		VK_RBUTTON, //        0x02
		VK_CANCEL,	//         0x03
	...
	constexpr int MAX_KEY_NUM = 255;
	static KeyCode keyCodeWin32ToShitTable[MAX_KEY_NUM];

	void InitEvent()
	{
		for (size_t i = 0, len = std::ranges::size(keyCodeArrayWin32); i < len; ++i)
			keyCodeWin32ToShitTable[keyCodeArrayWin32[i]] = static_cast<KeyCode>(i);
	}
	uint32_t MapKey(KeyCode keycode)
	{
		return keyCodeArrayWin32[static_cast<size_t>(keycode)];
	}
	KeyCode MapKey(uint32_t keycode)
	{
		return keyCodeWin32ToShitTable[static_cast<size_t>(keycode)];
	}
	...
	{
		auto a = ::GetKeyState(MapKey(keycode));
		return {bool(a >> 15), bool(a & 1)};
	}
