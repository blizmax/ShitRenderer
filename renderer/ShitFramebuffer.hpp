		...
		Framebuffer(const FramebufferCreateInfo &createInfo) : mCreateInfo(createInfo)
		{
			if (createInfo.attachmentCount)
			{
				if (mCreateInfo.pAttachments)
				{
					...
					memcpy((void *)mCreateInfo.pAttachments, createInfo.pAttachments, sizeof(std::ptrdiff_t) * mCreateInfo.attachmentCount);
				}
				else
				{
					mCreateInfo.pAttachmentImageInfos = new FramebufferAttachmentImageInfo[createInfo.attachmentCount];
					memcpy((void *)mCreateInfo.pAttachmentImageInfos,
						   createInfo.pAttachmentImageInfos,
					...
					for (uint32_t i = 0; i < createInfo.attachmentCount; ++i)
					{
						...
						if (p->viewFormatCount)
						{
							...
							memcpy((void *)p->pViewFormats, createInfo.pAttachmentImageInfos[i].pViewFormats, sizeof(Format) * p->viewFormatCount);
						}
					}
				}
			}
