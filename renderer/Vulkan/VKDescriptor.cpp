...
namespace Shit
{
	...
	void VKDescriptorPool::Allocate(const DescriptorSetAllocateInfo &allocateInfo, std::vector<DescriptorSet *> &descriptorSets)
	{
	...
	void VKDescriptorPool::Allocate(const DescriptorSetAllocateInfo &allocateInfo, DescriptorSet **ppDescriptorSets)
	{
		...
		{
			ppDescriptorSets[i] = *mDescriptorSets.emplace(new VKDescriptorSet(mpDevice, mHandle, allocateInfo.setLayouts[i])).first;
		}
	}
}