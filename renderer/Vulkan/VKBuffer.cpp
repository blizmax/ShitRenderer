...
namespace Shit
{
		...
		: Buffer(createInfo), VKDeviceObject(pDevice), mPhysicalDevice(physicalDevice)
	{
		...
		// ST_LOG_VAR(bufferMemoryRequirements.memoryTypeBits); //typebits is the memorytype indices in physical memory properties
	...
	void VKBuffer::Resize(uint64_t size)
	{
		...
		if (size > mMemorySize)
		{
			...
			auto memoryTypeIndex = VK::findMemoryTypeIndex(mPhysicalDevice,
	...
	void VKBuffer::UnMapMemory() const
	{
	...
	void VKBuffer::UpdateSubData(uint64_t offset, uint64_t size, void const *pData) const
	{
		...
		{
			ST_THROW("buffer should be created with a BufferUsageFlagBits::TRANSFER_DST_BIT or MemoryPropertyFlagBits::HOST_VISIBLE_BIT");
		}
	}
