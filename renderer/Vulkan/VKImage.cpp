...
namespace Shit
{
	...
	ImageLayout VKImage::Init()
	{
		if (mCreateInfo.mipLevels == 0)
			mCreateInfo.mipLevels = static_cast<uint32_t>(
				(std::floor)((std::log2)((std::max)((std::max)((float)mCreateInfo.extent.width,
															   (float)mCreateInfo.extent.height),
		...
		if (vkCreateImage(device, &imageCreateInfo, nullptr, &mHandle) != VK_SUCCESS)
			ST_THROW("failed to create image");
		...
		auto memoryTypeIndex = VK::findMemoryTypeIndex(static_cast<VKDevice *>(mpDevice)->GetPhysicalDevice(),
	...
	VKImage::VKImage(VKDevice *pDevice,
		...
		: Image(createInfo), VKDeviceObject(pDevice)
	{
		...
		{
			ST_LOG("when image data is not null, the initial layout cannot be undefined or preinitialized");
			exit(-1);
		}
		...
		{
			mpDevice->_ExecuteOneTimeCommand(
	...
	void VKImage::CopyBufferToImage(
	...
	{
		static_cast<VKDevice *>(mpDevice)->_ExecuteOneTimeCommand(
			[&](CommandBuffer *cmdBuffer)
			{
		...
		const void *pData) const
	{
		...
		uint64_t size = rect.extent.width *
							rect.extent.height *
							rect.extent.depth *
							GetFormatSize(mCreateInfo.format) -
						offset;
	...
	void VKImage::UnMapMemory() const
	{
	...
	void VKImage::InvalidateTexSubImage(
		[[maybe_unused]] int level,
		[[maybe_unused]] int xoffset,
		[[maybe_unused]] int yoffset,
		[[maybe_unused]] int zoffset,
	...
	void VKImage::GetImageSubresourceLayout(const ImageSubresource &subresouce, SubresourceLayout &subresourceLayout) const
	{
		VkSubresourceLayout ret;
		vkGetImageSubresourceLayout(
			static_cast<VKDevice *>(mpDevice)->GetHandle(),
			mHandle,
			reinterpret_cast<const VkImageSubresource *>(&subresouce),
			&ret);
