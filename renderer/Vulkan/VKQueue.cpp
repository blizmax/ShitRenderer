...
namespace Shit
{
	...
	{
		static auto v_semaphore = std::views::transform(
			[](const Semaphore *p)
			{ return static_cast<const VKSemaphore *>(p)->GetHandle(); });
		static auto v_commandBuffer = std::views::transform(
			[](const CommandBuffer *p)
		...
		static auto v_s = std::views::transform(
			[](const SubmitInfo &e) -> SubmitInfoCache
			{
				SubmitInfoCache ret;
				std::ranges::copy(std::span(e.pWaitSemaphores, e.waitSemaphoreCount) | v_semaphore, std::back_inserter(ret.waitSemaphores));
				std::ranges::copy(std::span(e.pSignalSemaphores, e.signalSemaphoreCount) | v_semaphore, std::back_inserter(ret.signalSemaphores));
				std::ranges::copy(std::span(e.pCommandBuffers, e.commandBufferCount) | v_commandBuffer, std::back_inserter(ret.commandBuffers));
				ret.waitDstStageMask.resize(e.waitSemaphoreCount, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT);
				return ret;
			});

		static auto v_final = std::views::transform(
			[](SubmitInfoCache &e)
			{
				return VkSubmitInfo{
	...
	Result VKQueue::Present(const PresentInfo &presentInfo)
	{
		static auto v_semaphore = std::views::transform(
			[](const Semaphore *p)
			{ return static_cast<const VKSemaphore *>(p)->GetHandle(); });
		static auto v_swapchain = std::views::transform(
			[](const Swapchain *p)
			{ return static_cast<const VKSwapchain *>(p)->GetHandle(); });

		std::vector<VkSemaphore> waitSemaphores;
		std::vector<VkSwapchainKHR> swapchains;
		std::ranges::copy(std::span(presentInfo.pWaitSemaphores, presentInfo.waitSemaphoreCount) | v_semaphore, std::back_inserter(waitSemaphores));
		std::ranges::copy(std::span(presentInfo.pSwapchains, presentInfo.swapchainCount) | v_swapchain, std::back_inserter(swapchains));
