...
namespace Shit
{
	...
	VKRenderSystem::VKRenderSystem(const RenderSystemCreateInfo &createInfo) : RenderSystem(createInfo)
	{
		...
		if (vkCreateInstance(&instanceInfo, 0, &mInstance) != VK_SUCCESS)
			ST_THROW("create instance failed");
	}

	std::unique_ptr<Surface> VKRenderSystem::CreateSurface(const SurfaceCreateInfo &createInfo, Window *pWindow)
	{
	...
	{
		static PhysicalDevice physicalDevice = VK::pickPhysicalDevice(mInstance);
