		...
		: RenderPass(createInfo), VKDeviceObject(device)
	{
		...
		for (uint32_t i = 0, j; i < mCreateInfo.subpassCount; ++i)
		{
			...
			subPasses[i] = {
				...
				e.inputAttachmentCount,
				0,
				e.colorAttachmentCount,
			};
			if (e.inputAttachmentCount)
				subPasses[i].pInputAttachments = attachments.data() + attachments.size();
			...
			if (e.colorAttachmentCount)
				subPasses[i].pColorAttachments = attachments.data() + attachments.size();
