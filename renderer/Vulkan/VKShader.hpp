...
namespace Shit
{
	...
	class VKShader final : public Shader, public VKDeviceObject
	{
		...
		{
			vkDestroyShaderModule(mpDevice->GetHandle(), mHandle, nullptr);
		}
		void Initialize() override
		{
			VkShaderModuleCreateInfo tempCreateInfo{
				...
				0,
				mCreateInfo.size,
				reinterpret_cast<const uint32_t *>(mCreateInfo.code)};
	...
	public:
		VKShader(VKDevice *device, const ShaderCreateInfo &createInfo)
			: Shader(createInfo), VKDeviceObject(device)
		{
			Initialize();
		}
		~VKShader() override
