...
namespace Shit
{
	namespace VK
	{
		...
		std::optional<uint32_t> findQueueFamilyIndexByFlag(
			std::span<const VkQueueFamilyProperties> queueFamilyProperties, VkQueueFlags flag)
		{
		...
		VkPhysicalDevice pickPhysicalDevice(VkInstance instance)
		{
			...
			if (temp.first == 0)
				ST_THROW("failed to find a suitable GPU");
    ...
    ST_LOG_VAR(memoryProperties.memoryTypeCount);
		...
		VkCommandBuffer beginOneTimeCommands(VkDevice logicalDevice, VkCommandPool commandPool)
		{
			...
			if (vkBeginCommandBuffer(commandBuffers[0], &beginInfo) != VK_SUCCESS)
				ST_THROW("failed to begin command buffer");
		...
		void endOneTimeCommands(VkDevice logicalDevice, VkQueue queue, VkCommandPool commandPool, VkCommandBuffer commandBuffer)
		{
			...
			if (vkQueueSubmit(queue, 1, &submitInfo, 0) != VK_SUCCESS)
				ST_THROW("failed to submit one time commands");
			vkQueueWaitIdle(queue);
			vkFreeCommandBuffers(logicalDevice, commandPool, 1, &commandBuffer);
		}
		VkFormat findSupportedFormat(VkPhysicalDevice physicalDevice, std::span<const VkFormat> candidates, VkImageTiling tiling, VkFormatFeatureFlags features)
		{
	...
	{
		return static_cast<VkPipelineCreateFlags>(flags);
	}
	...
	Format Map(VkFormat format)
	{
		...
		if (tempMap.find(format) == tempMap.end())
		{
			for (int i = 0; i < static_cast<int>(Format::Num); ++i)
			{
				...
				if (a == format)
					return tempMap[format] = static_cast<Format>(i);
			}
	...
	QueueFlagBits Map(VkQueueFlags flag)
	{
		...
		if (flag & VK_QUEUE_GRAPHICS_BIT)
			ret |= Shit::QueueFlagBits::GRAPHICS_BIT;
		if (flag & VK_QUEUE_TRANSFER_BIT)
			ret |= Shit::QueueFlagBits::TRANSFER_BIT;
		if (flag & VK_QUEUE_COMPUTE_BIT)
			ret |= Shit::QueueFlagBits::COMPUTE_BIT;
		if (flag & VK_QUEUE_SPARSE_BINDING_BIT)
			ret |= Shit::QueueFlagBits::SPARSE_BINDING_BIT;
		return ret;
	}
	VkFormatFeatureFlags Map(FormatFeatureFlagBits flags)
	{
		...
		{
			if (a & 1)
				ret |= vkFormatFeatureFlagBitsArray[i];
		}
		return ret;
	}
	...
	{
		return static_cast<VkStencilFaceFlags>(flags);
	}
