...
namespace Shit
{
	...
	public:
		VKFence(VKDevice *device, const FenceCreateInfo &createInfo)
			: Fence(createInfo), VKDeviceObject(device)
		{
		...
		~VKFence() override
		{
			vkDestroyFence(mpDevice->GetHandle(), mHandle, nullptr);
		}
		...
		{
			vkResetFences(mpDevice->GetHandle(), 1, &mHandle);
		}
