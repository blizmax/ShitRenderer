...
namespace Shit
{
	...
	{
		vkFreeCommandBuffers(mpDevice->GetHandle(),
							 static_cast<VKCommandPool *>(mCommandPool)->GetHandle(), 1, &mHandle);
	}
	...
	void VKCommandBuffer::Begin(const CommandBufferBeginInfo &beginInfo)
	{
		VkCommandBufferInheritanceInfo inheritancInfo{
			...
			nullptr,
			beginInfo.inheritanceInfo.pRenderPass ? static_cast<VKRenderPass const *>(beginInfo.inheritanceInfo.pRenderPass)->GetHandle() : VK_NULL_HANDLE,
			beginInfo.inheritanceInfo.subpass,
			beginInfo.inheritanceInfo.pFramebuffer ? static_cast<VKFramebuffer const *>(beginInfo.inheritanceInfo.pFramebuffer)->GetHandle() : VK_NULL_HANDLE,
		};
	...
	void VKCommandBuffer::ExecuteCommands(const ExecuteCommandsInfo &info)
	{
		...
		std::ranges::transform(info.pCommandBuffers, info.pCommandBuffers + info.count, std::back_inserter(cmdBuffers), [](auto p)
							   { return static_cast<const VKCommandBuffer *>(p)->GetHandle(); });
	...
	void VKCommandBuffer::BeginRenderPass(const BeginRenderPassInfo &beginInfo)
	{
		...
		VkRenderPassAttachmentBeginInfo attachmentBeginInfo{
			...
			0, beginInfo.attachmentBeginInfo.attachmentCount};
	...
	void VKCommandBuffer::CopyBuffer(const CopyBufferInfo &copyInfo)
	{
		vkCmdCopyBuffer(
			mHandle,
			static_cast<VKBuffer const *>(copyInfo.pSrcBuffer)->GetHandle(),
			static_cast<VKBuffer const *>(copyInfo.pDstBuffer)->GetHandle(),
	...
	void VKCommandBuffer::CopyImage(const CopyImageInfo &copyInfo)
	{
		vkCmdCopyImage(
			mHandle,
			static_cast<VKImage const *>(copyInfo.pSrcImage)->GetHandle(),
	...
	void VKCommandBuffer::BlitImage(const BlitImageInfo &blitInfo)
	{
		vkCmdBlitImage(
			mHandle,
			static_cast<VKImage const *>(blitInfo.pSrcImage)->GetHandle(),
			VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
			static_cast<VKImage const *>(blitInfo.pDstImage)->GetHandle(),
			VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
			blitInfo.regionCount,
			reinterpret_cast<const VkImageBlit *>(blitInfo.pRegions),
			Map(blitInfo.filter));
	}
	void VKCommandBuffer::BindVertexBuffers(const BindVertexBuffersInfo &info)
	{
		std::vector<VkBuffer> buffers;
		static auto vt = std::views::transform(
			...
			for (; it1 != end; ++it1)
			{
				if (*it1 == VK_NULL_HANDLE)
				{
					if (it1 != it0)
					{
						...
						vkCmdBindVertexBuffers(
							mHandle,
							info.firstBinding + offset,
							static_cast<uint32_t>(std::distance(it0, it1)),
							&buffers[offset],
	...
	void VKCommandBuffer::BindIndexBuffer(const BindIndexBufferInfo &info)
	{
		vkCmdBindIndexBuffer(
			mHandle,
			static_cast<VKBuffer const *>(info.pBuffer)->GetHandle(),
			info.offset,
			Map(info.indexType));
	}
	void VKCommandBuffer::BindDescriptorSets(const BindDescriptorSetsInfo &info)
	{
		...
		std::ranges::transform(
			info.ppDescriptorSets,
			info.ppDescriptorSets + info.descriptorSetCount,
			std::back_inserter(descriptorSets),
			[](auto p)
			{
				return static_cast<const VKDescriptorSet *>(p)->GetHandle();
			});
	...
	void VKCommandBuffer::Draw(const DrawIndexedIndirectCommand &info)
	{
		DrawIndexed(info);
	}
	...
	void VKCommandBuffer::DrawIndirect(const DrawIndirectCountInfo &info)
	{
	...
	void VKCommandBuffer::DrawIndirectCount([[maybe_unused]] const DrawIndirectCountInfo &info)
	{
		...
		vkCmdDrawIndirectCount(
			...
			static_cast<VKBuffer const *>(info.pBuffer)->GetHandle(),
			info.offset,
			static_cast<VKBuffer const *>(info.pCountBuffer)->GetHandle(),
	...
	void VKCommandBuffer::DrawIndexedIndirect(const DrawIndirectCountInfo &info)
	{
	...
	void VKCommandBuffer::ClearColorImage(const ClearColorImageInfo &info)
	{
		vkCmdClearColorImage(
			mHandle,
			static_cast<VKImage const *>(info.image)->GetHandle(),
			Map(info.imageLayout),
			reinterpret_cast<const VkClearColorValue *>(&info.clearColor),
			info.rangeCount,
	...
	void VKCommandBuffer::ClearDepthStencilImage(const ClearDepthStencilImageInfo &info)
	{
		vkCmdClearDepthStencilImage(
			...
			static_cast<VKImage const *>(info.image)->GetHandle(),
			Map(info.imageLayout),
			reinterpret_cast<const VkClearDepthStencilValue *>(&info.depthStencil),
			info.rangeCount,
			reinterpret_cast<const VkImageSubresourceRange *>(info.pRanges));
	}
	void VKCommandBuffer::ClearAttachments(const ClearAttachmentsInfo &info)
	{
		vkCmdClearAttachments(
			...
			reinterpret_cast<const VkClearAttachment *>(info.pAttachments),
			info.rectCount,
			reinterpret_cast<const VkClearRect *>(info.pRects));
	}
	void VKCommandBuffer::FillBuffer(const FillBufferInfo &info)
	{
		vkCmdFillBuffer(mHandle, static_cast<VKBuffer const *>(info.buffer)->GetHandle(), info.offset, info.size, info.data);
	}
	void VKCommandBuffer::UpdateBuffer(const UpdateBufferInfo &info)
	{
		vkCmdUpdateBuffer(mHandle, static_cast<VKBuffer const *>(info.dstBuffer)->GetHandle(), info.dstOffset, info.dataSize, info.pData);
	}
	...
	void VKCommandBuffer::PushConstants(const PushConstantInfo &info)
	{
		vkCmdPushConstants(
			mHandle,
			static_cast<VKPipelineLayout const *>(info.pPipelineLayout)->GetHandle(),
			Map(info.stageFlags),
			info.offset,
			info.size,
			info.pValues);
	...
	void VKCommandBuffer::SetViewport(const SetViewPortInfo &info)
	{
		...
		for (uint32_t i = 0; i < info.viewportCount; ++i)
			viewports[i] = {
				info.pViewports[i].x,
				info.pViewports[i].height - info.pViewports[i].y,
	...
	void VKCommandBuffer::ResolveImage(const ResolveImageInfo &info)
	{
		...
		for (uint32_t i = 0; i < info.regionCount; ++i)
		{
			...
			imageResolves[i] = {
				{Map(e.srcSubresource.aspectMask),
				...
				{Map(e.dstSubresource.aspectMask),
		...
		vkCmdResolveImage(
			mHandle,
			static_cast<VKImage const *>(info.pSrcImage)->GetHandle(),
			Map(info.srcImageLayout),
			static_cast<VKImage const *>(info.pDstImage)->GetHandle(),
			Map(info.dstImageLayout),
			info.regionCount,
			imageResolves.data());
	}
