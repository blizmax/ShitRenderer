...
namespace Shit
{
		...
		: PipelineLayout(createInfo), VKDeviceObject(device)
	{
		...
		for (uint32_t i = 0; i < mCreateInfo.setLayoutCount; ++i)
			setLayouts.emplace_back(static_cast<const VKDescriptorSetLayout *>(mCreateInfo.pSetLayouts[i])->GetHandle());
	...
	//=================================VKPipeline=======================
	void VKPipeline::ConvertPipelineShaderStageInfo(uint32_t stageCount,
	...
	void VKGraphicsPipeline::Initialize()
	{
		...
		//ConvertPipelineShaderStageInfo(mCreateInfo.stageCount, mCreateInfo.stages, shaderStageCreateInfos);
		...
		for (uint32_t i = 0; i < vertexInputBindingCount; ++i)
		{
			...
			vertexInputBindingDescs.emplace_back(e.binding, e.stride, e.divisor ? VK_VERTEX_INPUT_RATE_INSTANCE : VK_VERTEX_INPUT_RATE_VERTEX);
		}
		...
		VkStencilOpState frontStencilOpState{
			Map(mCreateInfo.depthStencilState.front.failOp),
			Map(mCreateInfo.depthStencilState.front.passOp),
			Map(mCreateInfo.depthStencilState.front.depthFailOp),
			Map(mCreateInfo.depthStencilState.front.compareOp),
			mCreateInfo.depthStencilState.front.compareMask,
			mCreateInfo.depthStencilState.front.writeMask,
			mCreateInfo.depthStencilState.front.reference,
		};
		VkStencilOpState backStencilOpState{
			Map(mCreateInfo.depthStencilState.back.failOp),
			Map(mCreateInfo.depthStencilState.back.passOp),
			Map(mCreateInfo.depthStencilState.back.depthFailOp),
			Map(mCreateInfo.depthStencilState.back.compareOp),
			mCreateInfo.depthStencilState.back.compareMask,
			mCreateInfo.depthStencilState.back.writeMask,
		...
		bool dyanmicStateViewportWidthCount = false,
			 dyanmicStateScissorWidthCount = false;
		...
		{
			scissors[i] = {
				{mCreateInfo.viewportState.scissors[i].offset.x,
				 mCreateInfo.viewportState.scissors[i].offset.y},
				{mCreateInfo.viewportState.scissors[i].extent.width,
				 mCreateInfo.viewportState.scissors[i].extent.height}};
		}
		...
		VkGraphicsPipelineCreateInfo pipelineCreateInfo{
			...
			mCreateInfo.pLayout ? static_cast<VKPipelineLayout *>(mCreateInfo.pLayout)->GetHandle() : VK_NULL_HANDLE,
			mCreateInfo.pRenderPass ? static_cast<VKRenderPass *>(mCreateInfo.pRenderPass)->GetHandle() : VK_NULL_HANDLE,
			mCreateInfo.subpass, //subpass index in renderpass
		};
		CHECK_VK_RESULT(vkCreateGraphicsPipelines(mpDevice->GetHandle(),
												  VK_NULL_HANDLE,
												  1,
												  &pipelineCreateInfo,
												  nullptr,
