...
#define CHECK_VK_RESULT(x)                         \
	{                                              \
		auto res = x;                              \
		if (res != VK_SUCCESS)                     \
			ST_THROW(#x " failed, error code:", res); \
	}
...
namespace Shit
{
	...
	namespace VK
	{
		 ...
		 */
		std::optional<uint32_t> findQueueFamilyIndexByFlag(std::span<const VkQueueFamilyProperties> queueFamilyProperties, VkQueueFlags flag);
