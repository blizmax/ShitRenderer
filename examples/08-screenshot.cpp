		...
		std::visit(Shit::overloaded{
					   [&](const Shit::KeyEvent &value)
					   {
						   if (value.keyCode == Shit::KeyCode::KEY_ESCAPE)
							   ev.pWindow->Close();
						   if (value.keyCode == Shit::KeyCode::KEY_P && value.state)
...
";
	...
	void createSwapchain()
	{
		...
		swapchainCreateInfo = Shit::SwapchainCreateInfo{
			...
			{800, 600},
			1,
			Shit::ImageUsageFlagBits::TRANSFER_SRC_BIT | Shit::ImageUsageFlagBits::COLOR_ATTACHMENT_BIT,
			presentMode,
			window};
	...
	void createDescriptorSets()
	{
		...
		descriptorSetLayouts.resize(
			imageCount, device->Create(Shit::DescriptorSetLayoutCreateInfo{(uint32_t)bindings.size(), bindings.data()}));
	...
	void createImages()
	{
		...
		testImage->GenerateMipmap(Shit::Filter::LINEAR, Shit::ImageLayout::SHADER_READ_ONLY_OPTIMAL, Shit::ImageLayout::SHADER_READ_ONLY_OPTIMAL);
