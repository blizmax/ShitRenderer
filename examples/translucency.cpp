...
static char const *imagePaths[]{
	SHIT_SOURCE_DIR "/assets/images/Arches_E_PineTree_3k.hdr",
	...
	SHIT_SOURCE_DIR "/assets/images/Mt-Washington-Cave-Room_Ref.hdr",
	SHIT_SOURCE_DIR "/assets/images/Mt-Washington-Gold-Room_Ref.hdr",
	SHIT_SOURCE_DIR "/assets/images/Ridgecrest_Road_Ref.hdr",
};
...
class ModelController : public Behaviour
{
	...
	{
		getParentTransform()->yaw(glm::radians(frameDtMs / 1000 * _anglePerSecond),
...
public:
	void loadAssets()
	{
		...
		{
			models.emplace_back(static_cast<Model *>(_modelManager->create(e)))
		...
		envBRDFTexSampler = createSampler(
			"envBRDFTexSampler",
		...
		{
			auto image = counterImages.emplace_back(
				_device->Create(Shit::ImageCreateInfo{
									{},
			...
			counterImageViews.emplace_back(_device->Create(Shit::ImageViewCreateInfo{
				image,
			...
			image = outImages.emplace_back(
				_device->Create(Shit::ImageCreateInfo{
									...
									Shit::ImageLayout::GENERAL},
								Shit::ImageAspectFlagBits::COLOR_BIT, 0));
			outImageViews.emplace_back(_device->Create(Shit::ImageViewCreateInfo{
				image,
	...
	void createSkyboxRenderPass()
	{
		...
		skyboxRenderTarget.renderPass = _device->Create(
	...
	void createSkyboxRenderTarget()
	{
		...
		for (uint32_t i = 0; i < frameCount; ++i)
		{
			auto pImageView = skyboxRenderTarget.imageViews.emplace_back(_device->Create(Shit::ImageViewCreateInfo{
			...
			skyboxRenderTarget.frameBuffers.emplace_back(_device->Create(
				Shit::FramebufferCreateInfo{
					skyboxRenderTarget.renderPass,
					1,
					&pImageView,
	...
	void preparePipelineInfos()
	{
		auto frameCount = getFramesInFlight();
		{
			...
			auto setLayout = _device->Create(Shit::DescriptorSetLayoutCreateInfo{(uint32_t)std::size(bindings), bindings});
			...
			_descriptorPool->Allocate({(uint32_t)tempSetLayouts.size(), tempSetLayouts.data()}, translucencyPipelineInfo.descriptorSets.data());
			...
			//===============================
			translucencyPipelineInfo.shaders = {
				static_cast<Shader *>(_shaderManager->create(
					buildShaderPath(_device, vertShaderPath0, _rendererVersion), ResourceManager::DEFAULT_LOADER_NAME, {{"stage", "VERT"}})),
				static_cast<Shader *>(_shaderManager->create(
					buildShaderPath(_device, fragShaderPath0, _rendererVersion), ResourceManager::DEFAULT_LOADER_NAME, {{"stage", "FRAG"}}))};
		}
		{
			Shit::DescriptorSetLayoutBinding bindings[]{
				{0, Shit::DescriptorType::STORAGE_IMAGE, 1, Shit::ShaderStageFlagBits::FRAGMENT_BIT},
			...
			skyboxPipelineInfo.pipelineLayout = _device->Create(
			...
			skyboxPipelineInfo.shaders = {
				static_cast<Shader *>(_shaderManager->create(
					buildShaderPath(_device, vertShaderPath1, _rendererVersion), ResourceManager::DEFAULT_LOADER_NAME, {{"stage", "VERT"}})),
				static_cast<Shader *>(_shaderManager->create(
					buildShaderPath(_device, fragShaderPath1, _rendererVersion), ResourceManager::DEFAULT_LOADER_NAME, {{"stage", "FRAG"}}))};
		}
	}
	...
	void updateDescriptorSets()
	{
		...
		for (uint32_t i = 0; i < frameCount; ++i)
		{
			...
			imagesInfo[i * imageCount + 2] = {trilinearSampler, _environment->getEnvTex()->getImageView(), Shit::ImageLayout::GENERAL};
			imagesInfo[i * imageCount + 3] = {envBRDFTexSampler, envBRDFTex->getImageView(), Shit::ImageLayout::SHADER_READ_ONLY_OPTIMAL};

			writes.emplace_back(Shit::WriteDescriptorSet{
				translucencyPipelineInfo.descriptorSets[i],
				0,
				0,
				1,
			...
			writes.emplace_back(Shit::WriteDescriptorSet{
				skyboxPipelineInfo.descriptorSets[i],
				0,
				0,
				1,
				Shit::DescriptorType::STORAGE_IMAGE,
	...
	void drawGameObject(Shit::CommandBuffer *cmdBuffer,
	...
	void recordDefaultCommandBuffer() override
	{
		...
		for (int i = 0; i < frameCount; ++i)
		{
			...
			cmdBuffer->ClearColorImage({counterImages[i],
										Shit::ImageLayout::GENERAL,
										Shit::ClearColorValueUint32{0},
										1,
										&range});
			...
			cmdBuffer->BindDescriptorSets(Shit::BindDescriptorSetsInfo{
				Shit::PipelineBindPoint::GRAPHICS,
				pipeline0->GetCreateInfoPtr()->pLayout,
				1,
				1,
				&cameraDescriptorSet});
			...
			for (auto &&e : _gameObjects)
				drawGameObject(cmdBuffer, e.get(), pipeline0->GetCreateInfoPtr()->pLayout, 2);
			...
			cmdBuffer->BindDescriptorSets(Shit::BindDescriptorSetsInfo{
				Shit::PipelineBindPoint::GRAPHICS,
				pipeline1->GetCreateInfoPtr()->pLayout,
				1,
				1,
				&cameraDescriptorSet});
			cmdBuffer->BindDescriptorSets(Shit::BindDescriptorSetsInfo{
			...
			cmdBuffer->PushConstants({skyboxPipelineInfo.pipelineLayout,
			...
			cmdBuffer->BeginRenderPass(
				Shit::BeginRenderPassInfo{
					skyboxRenderTarget.renderPass,
					skyboxRenderTarget.frameBuffers[i],
					{{}, _swapchain->GetCreateInfoPtr()->imageExtent},
	...
	void setupImGui()
	{
		...
		static const char *eqImages[]{
			"Arches_E_PineTree_3k.hdr",
			"ennis.hdr",
			"Footprint_Court_2k.hdr",
			"grace-new.hdr",
			"Mt-Washington-Cave-Room_Ref.hdr",
			"Mt-Washington-Gold-Room_Ref.hdr",
			"Ridgecrest_Road_Ref.hdr",
		};
		static int imageIndex = 0;
		const char *eqImageName = eqImages[imageIndex]; // Label to preview before opening the combo (technically it could be anything)
		if (ImGui::BeginCombo("skybox image", eqImageName))
		{
			for (int i = 0; i < std::size(eqImages); ++i)
			{
				const bool is_selected = imageIndex == i;
				if (ImGui::Selectable(eqImages[i], is_selected))
				{
					imageIndex = i;
		...
		_needUpdateDefaultCommandBuffer |= ImGui::ColorEdit3("extinction", uPC.extinction, ImGuiColorEditFlags_NoLabel | misc_flags);
	...
	void recreateSwapchainListener()
	{
		...
		for (uint32_t i = 0; i < frameCount; ++i)
		{
			...
			counterImageViews[i] = _device->Create(Shit::ImageViewCreateInfo{
				counterImages[i],
				Shit::ImageViewType::TYPE_2D,
				Shit::Format::R32_UINT,
				{},
			...
			outImages[i] =
				_device->Create(Shit::ImageCreateInfo{
									...
									Shit::ImageLayout::GENERAL},
								Shit::ImageAspectFlagBits::COLOR_BIT, 0);
			outImageViews[i] = _device->Create(Shit::ImageViewCreateInfo{
				outImages[i],
				Shit::ImageViewType::TYPE_2D_ARRAY,
				Shit::Format::R32G32_UINT,
				{},
		...
		for (auto e : skyboxRenderTarget.imageViews)
			_device->Destroy(e);
		skyboxRenderTarget.imageViews.clear();
		for (auto e : skyboxRenderTarget.frameBuffers)
			_device->Destroy(e);
