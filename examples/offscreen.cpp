...
void main() { \n\
	...
	void createRenderPasses()
	{
		...
		Shit::RenderPassCreateInfo renderPassCreateInfo{
			ARR_SIZE(attachmentDescriptions),
	...
	void createPipeline()
	{
		...
		Shit::GraphicsPipelineCreateInfo pipelineCreateInfo{
			ARR_SIZE(shaderStageCreateInfos),
	...
	void createOffscreenImages()
	{
		...
		auto depthFormat = device->GetSuitableImageFormat(
			formats,
		...
		offscreenImageViews[0] = device->Create(
		...
		offscreenImageViews[1] = device->Create(
			Shit::ImageViewCreateInfo{
				offscreenImages[1],
				Shit::ImageViewType::TYPE_2D,
				offscreenImages[1]->GetCreateInfoPtr()->format,
	...
	void createFramebuffers()
	{
		Shit::FramebufferCreateInfo framebufferCreateInfo{
			renderPass,
			(uint32_t)offscreenImageViews.size(),
			offscreenImageViews.data(),
			{WIDTH, HEIGHT},
			1};
