...
class LightController : public Behaviour
{
	...
	{
		getParentTransform()->translate({-12., 3., 3.})->yaw(glm::radians(-90.f));
	}
...
public:
	...
	void writeDescriptorSets()
	{
		...
		for (int i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i)
		{
			...
			writeSets.emplace_back(
				Shit::WriteDescriptorSet{pipelineInfo.descriptorSets[i], 8, 0, 1, Shit::DescriptorType::COMBINED_IMAGE_SAMPLER, &imageInfo[0]});
			writeSets.emplace_back(
				Shit::WriteDescriptorSet{pipelineInfo.descriptorSets[i], 9, 0, 1, Shit::DescriptorType::COMBINED_IMAGE_SAMPLER, &imageInfo[1]});
		};
	...
	void writeShowTexDescriptorSets()
	{
		...
		Shit::DescriptorImageInfo imageInfo{sampler, shadowRenderTarget.imageViews[0], Shit::ImageLayout::SHADER_READ_ONLY_OPTIMAL};
		std::vector<Shit::WriteDescriptorSet> writeSets{
	...
	void setupImGui()
	{
		...
		ImGui::Separator();
		{
			...
			static glm::vec3 lightRotation = glm::degrees(glm::eulerAngles(light->getParentTransform()->getLocalRotation()));
