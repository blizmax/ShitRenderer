...
public:
		...
		for (uint32_t i = 0; i < frameCount; ++i)
		{
			atomicBuffers.emplace_back(
				_device->Create(
					...
					0));
			auto listBuffer = listBuffers.emplace_back(_device->Create(Shit::BufferCreateInfo{
				{},
				listBufferSize,
				Shit::BufferUsageFlagBits::STORAGE_TEXEL_BUFFER_BIT,
				Shit::MemoryPropertyFlagBits::DEVICE_LOCAL_BIT}));
			listBufferViews.emplace_back(_device->Create(Shit::BufferViewCreateInfo{
				listBuffer,
				Shit::Format::R32G32B32A32_UINT,
				0,
				listBufferSize}));
			auto image = headImages.emplace_back(
			...
			headImageViews.emplace_back(_device->Create(Shit::ImageViewCreateInfo{
				image,
	...
	void updateDescriptorSets()
	{
		...
		for (uint32_t i = 0; i < frameCount; ++i)
		{
			...
			writes.emplace_back(Shit::WriteDescriptorSet{
				pipelineInfo0.descriptorSets[i],
				0,
				0,
				1,
			...
			writes.emplace_back(Shit::WriteDescriptorSet{
				...
				0,
				1,
				Shit::DescriptorType::STORAGE_TEXEL_BUFFER,
				0,
				0,
				&listBufferViews[i]});
			writes.emplace_back(Shit::WriteDescriptorSet{
				...
				0,
				1,
				Shit::DescriptorType::STORAGE_BUFFER,
				0,
				&buffersInfo[i]});
	...
	void recordDefaultCommandBuffer() override
	{
		...
		for (int i = 0; i < frameCount; ++i)
		{
			...
			cmdBuffer->FillBuffer({atomicBuffers[i], 0, atomicBuffers[i]->GetCreateInfoPtr()->size, 0});
			Shit::ImageSubresourceRange range{Shit::ImageAspectFlagBits::COLOR_BIT, 0, 1, 0, 1};
			cmdBuffer->ClearColorImage({headImages[i],
										Shit::ImageLayout::GENERAL,
										Shit::ClearColorValueUint32{0xFFFFFFFF},
										1,
										&range});
		...
		for (uint32_t i = 0; i < frameCount; ++i)
		{
			...
			headImages[i] =
				_device->Create(Shit::ImageCreateInfo{
									...
									Shit::ImageLayout::GENERAL},
								Shit::ImageAspectFlagBits::COLOR_BIT, -1);
			headImageViews[i] = _device->Create(Shit::ImageViewCreateInfo{
				headImages[i],
				Shit::ImageViewType::TYPE_2D,
				Shit::Format::R32_UINT,
				{},
