...
public:
	...
	void createEq2CubePipeline()
	{
		...
		eq2cubePipelineInfo.pipelineLayout = _device->Create(
		...
		eq2cubePipelineInfo.pipeline = _device->Create(
			Shit::ComputePipelineCreateInfo{shaderStageCI, eq2cubePipelineInfo.pipelineLayout});
	}
	void createCubemp()
	{
		...
		executeOneTimeCommands(
			...
			[&](Shit::CommandBuffer *cmdBuffer)
			{
				...
				cmdBuffer->BindDescriptorSets(Shit::BindDescriptorSetsInfo{
					Shit::PipelineBindPoint::COMPUTE,
					eq2cubePipelineInfo.pipelineLayout,
					0,
					1,
					&eq2cubeDescriptorSet});
	...
	void createCubemapMipmapPipeline()
	{
		...
		cubemapMipmapPipelineInfo.pipelineLayout = _device->Create(
			Shit::PipelineLayoutCreateInfo{1, &descriptorSetLayout});

		cubemapMipmapPipelineInfo.shaders = {
			static_cast<Shader *>(_shaderManager->create(
				buildShaderPath(_device, cubemapMipmapCompShaderName, _rendererVersion),
				ResourceManager::DEFAULT_LOADER_NAME, {{"stage", "COMP"}}))};

		cubemapMipmapPipelineInfo.shaders[0]->load();
		Shit::PipelineShaderStageCreateInfo shaderStageCI{
			cubemapMipmapPipelineInfo.shaders[0]->getStage(),
			cubemapMipmapPipelineInfo.shaders[0]->getHandle(),
			cubemapMipmapPipelineInfo.shaders[0]->getEntryName().data()};

		cubemapMipmapPipelineInfo.pipeline = _device->Create(
			Shit::ComputePipelineCreateInfo{shaderStageCI, cubemapMipmapPipelineInfo.pipelineLayout});
	}
	void generateCubemapMipmap()
	{
		...
		{
			cubeTexImageViews.emplace_back(_device->Create(Shit::ImageViewCreateInfo{
				cubeTex->getImage(),
				Shit::ImageViewType::TYPE_CUBE,
				cubeTex->getImage()->GetCreateInfoPtr()->format,
				{},
				{Shit::ImageAspectFlagBits::COLOR_BIT, i, 1, 0, cubeTex->getImage()->GetCreateInfoPtr()->arrayLayers}}));
		}
		for (uint32_t i = 1; i < mipLevels; ++i)
		{
			...
			Shit::WriteDescriptorSet writeSets[]{
				{cubemapMipmapDescriptorSets[i - 1],
				 0,
				 0,
				 1,
				 Shit::DescriptorType::COMBINED_IMAGE_SAMPLER,
	...
	void createSkyboDescriptorSet()
	{
		_descriptorPool->Allocate(
			Shit::DescriptorSetAllocateInfo{
				1, &skyboxPipelineInfo.pipelineLayout->GetCreateInfoPtr()->pSetLayouts[0]},
			&skyboxDescriptorSet);
	...
	void recordDefaultCommandBuffer() override
	{
		...
		for (int i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i)
		{
			...
			for (auto &&e : _gameObjects)
				drawGameObject(cmdBuffer, e.get(), pipelineInfo.pipelineLayout);
