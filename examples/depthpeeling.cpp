...
public:
	...
	void createDefaultRenderTarget() override
	{
		...
		for (int i = 0; i < frameCount; ++i)
		{
			...
			auto col1 = _defaultRenderTarget.imageViews.emplace_back(_device->Create(colorImageViewCI));
	...
	void preparePipelineInfos()
	{
		auto frameCount = getFramesInFlight();
		{
			...
			_descriptorPool->Allocate({(uint32_t)tempSetLayouts.size(), tempSetLayouts.data()}, pipelineInfo0.descriptorSets.data());
	...
	void updateDescriptorSets()
	{
		...
		for (uint32_t i = 0; i < frameCount; ++i)
		{
			imagesInfo[i * 3] = {0, _defaultRenderTarget.imageViews[i * attachmentCount + 2], Shit::ImageLayout::GENERAL};
			imagesInfo[i * 3 + 1] = {0, _defaultRenderTarget.imageViews[i * attachmentCount + 1], Shit::ImageLayout::GENERAL};
			imagesInfo[i * 3 + 2] = {0, _defaultRenderTarget.imageViews[i * attachmentCount + 0], Shit::ImageLayout::GENERAL};

			writes.emplace_back(Shit::WriteDescriptorSet{
				pipelineInfo0.descriptorSets[i * 2],
				8,
				0,
				1,
				...
				&imagesInfo[i * 3]});
			writes.emplace_back(Shit::WriteDescriptorSet{
				pipelineInfo0.descriptorSets[i * 2 + 1],
				8,
				0,
				1,
				Shit::DescriptorType::INPUT_ATTACHMENT,
				&imagesInfo[i * 3 + 1]});
			writes.emplace_back(Shit::WriteDescriptorSet{
				pipelineInfo1.descriptorSets[i],
				0,
				0,
				1,
				Shit::DescriptorType::INPUT_ATTACHMENT,
	...
	void recordDefaultCommandBuffer() override
	{
		...
		for (int i = 0; i < frameCount; ++i)
		{
			...
			auto imageBarrier = Shit::ImageMemoryBarrier{
				{},
			...
			cmdBuffer->ClearColorImage({_defaultRenderTarget.images[i * attachmentCount + 3],
										Shit::ImageLayout::TRANSFER_DST_OPTIMAL,
			...
			cmdBuffer->ClearDepthStencilImage({_defaultRenderTarget.images[i * attachmentCount + 2],
											   Shit::ImageLayout::GENERAL,
											   Shit::ClearDepthStencilValue{0, 0},
											   1, &depthRange});
			...
			for (int j = 0; j < layerCount; ++j)
			{
				...
				cmdBuffer->BeginRenderPass(
					Shit::BeginRenderPassInfo{
						_defaultRenderTarget.renderPass,
						_defaultRenderTarget.frameBuffers[i * 2 + k],
						{{}, _swapchain->GetCreateInfoPtr()->imageExtent},
				...
				cmdBuffer->BindDescriptorSets(Shit::BindDescriptorSetsInfo{
					...
					4,
					1,
					&pipelineInfo0.descriptorSets[i * 2 + k]});
