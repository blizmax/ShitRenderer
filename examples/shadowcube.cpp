...
class LightController : public Behaviour
{
	...
	{
		getParentTransform()->translate({5., 3., 0.});
	}
...
public:
	...
	void writeShowTexDescriptorSets()
	{
		...
		auto imageView = _device->Create(
			Shit::ImageViewCreateInfo{
				shadowRenderTarget.imageViews[0]->GetCreateInfoPtr()->pImage,
	...
	void setupImGui()
	{
		...
		ImGui::Separator();
		{
			...
			if (viewMode == 0)
			{
				...
				updateUBOBuffer |= ImGui::SliderFloat("shadowFilterRadius", &uboDisplayData.shadowFilterRadius, 0.f, 10.f);
				if (updateUBOBuffer)
				{
