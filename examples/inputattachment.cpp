...
public:
	...
	void createDefaultRenderPass() override
	{
		...
		_defaultRenderTarget.clearValues = {
			clearColor, clearColor, clearDepthStencil};
	}
	...
	void prepareReadPipeline()
	{
		...
		_descriptorPool->Allocate(Shit::DescriptorSetAllocateInfo{(uint32_t)descriptorSetLayouts.size(), descriptorSetLayouts.data()},
	...
	void prepareDescriptorSets()
	{
		...
		uboReadBuffer = _device->Create(Shit::BufferCreateInfo{
											{},
											sizeof(UBORead),
	...
	void updateDescriptorSets()
	{
		...
		Shit::DescriptorImageInfo imagesInfo[2 * MAX_FRAMES_IN_FLIGHT];
		...
		for (uint32_t i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i)
		{
			...
			depthInputAttachmentImageView[i] = _device->Create(
			...
			imagesInfo[i] = Shit::DescriptorImageInfo{
				0,
				_defaultRenderTarget.imageViews[i * attachmentCount + 1],
				Shit::ImageLayout::SHADER_READ_ONLY_OPTIMAL};
			imagesInfo[i + MAX_FRAMES_IN_FLIGHT] = Shit::DescriptorImageInfo{
				0,
				depthInputAttachmentImageView[i],
				Shit::ImageLayout::SHADER_READ_ONLY_OPTIMAL};

			writes.emplace_back(Shit::WriteDescriptorSet{
				readDescriptorSets[i],
				0,
				0,
				1,
			...
			writes.emplace_back(Shit::WriteDescriptorSet{
				...
				1,
				Shit::DescriptorType::INPUT_ATTACHMENT,
				&imagesInfo[i + MAX_FRAMES_IN_FLIGHT]});
			writes.emplace_back(Shit::WriteDescriptorSet{
				readDescriptorSets[i],
				2,
				0,
	...
	void recordDefaultCommandBuffer() override
	{
		...
		for (int i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i)
		{
			...
			auto writePipelineLayout = dynamic_cast<Shit::GraphicsPipeline *>(writePipelineInfo.pipeline)->GetCreateInfoPtr()->pLayout;
			// bind camera
			...
			for (auto &&e : _gameObjects)
				drawGameObject(cmdBuffer, e.get(), writePipelineLayout);
			...
			cmdBuffer->BindDescriptorSets(Shit::BindDescriptorSetsInfo{
				Shit::PipelineBindPoint::GRAPHICS,
				dynamic_cast<Shit::GraphicsPipeline *>(readPipelineInfo.pipeline)->GetCreateInfoPtr()->pLayout,
				0,
				1,
				&readDescriptorSets[i]});
