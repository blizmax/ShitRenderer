...
class LightController : public Behaviour
{
	...
	{
		getParentTransform()->translate({0, 3, 0});
	}
...
public:
	...
	void createGpassRenderPass()
	{
		...
		gpassRenderTarget.renderPass = _device->Create(
	...
	void createDefaultRenderPass() override
	{
		...
		Shit::SubpassDescription subpass{Shit::PipelineBindPoint::GRAPHICS,
										 0,
										 0,
										 1,
	...
	void createDefaultRenderTarget() override
	{
		...
		for (int i = 0; i < getFramesInFlight(); ++i)
		{
			...
			colorImageViewCI.format = colorImageViewCI.pImage->GetCreateInfoPtr()->format;
			...
			_defaultRenderTarget.frameBuffers.emplace_back(_device->Create(
	...
	{
		{
			...
			gpassPipelineInfo.shaders = {
				static_cast<Shader *>(_shaderManager->create(
					buildShaderPath(_device, writeVertShaderPath, _rendererVersion), ResourceManager::DEFAULT_LOADER_NAME, {{"stage", "VERT"}})),
				static_cast<Shader *>(_shaderManager->create(
					buildShaderPath(_device, writeFragShaderPath, _rendererVersion), ResourceManager::DEFAULT_LOADER_NAME, {{"stage", "FRAG"}}))};
			...
			gpassPipelineInfo.pipelineLayout = _device->Create(
				Shit::PipelineLayoutCreateInfo{
					(uint32_t)std::size(descriptorSetLayouts),
		...
		}
		{
			...
			aoPipelineInfo.shaders = {
				static_cast<Shader *>(_shaderManager->create(
					buildShaderPath(_device, aoCompShaderPath, _rendererVersion), ResourceManager::DEFAULT_LOADER_NAME, {{"stage", "COMP"}})),
			};
			...
			auto pcRange = Shit::PushConstantRange{
				Shit::ShaderStageFlagBits::COMPUTE_BIT,
				2,
				0,
				sizeof(AOPushConstant)};
			setLayouts = {
				descriptorSetLayout,
				_cameraDescriptorSetLayout};
		...
		}
		{
			...
			shadingPipelineInfo.shaders = {
				static_cast<Shader *>(_shaderManager->create(
					buildShaderPath(_device, shadingCompShaderPath, _rendererVersion), ResourceManager::DEFAULT_LOADER_NAME, {{"stage", "COMP"}})),
			};
			Shit::DescriptorSetLayoutBinding bindings[]{
				// set 1
				{0, Shit::DescriptorType::COMBINED_IMAGE_SAMPLER, 1, Shit::ShaderStageFlagBits::COMPUTE_BIT},
			...
			auto pcRange = Shit::PushConstantRange{Shit::ShaderStageFlagBits::COMPUTE_BIT, 0, 0, sizeof(ShadingPushConstant)};
		...
		}
		{
			...
			postProcessPipelineInfo.shaders = {
				static_cast<Shader *>(_shaderManager->create(
					buildShaderPath(_device, readVertShaderPath, _rendererVersion), ResourceManager::DEFAULT_LOADER_NAME, {{"stage", "VERT"}})),
				static_cast<Shader *>(_shaderManager->create(
					buildShaderPath(_device, readFragShaderPath, _rendererVersion), ResourceManager::DEFAULT_LOADER_NAME, {{"stage", "FRAG"}}))};
			...
			postProcessPipelineInfo.pipelineLayout = _device->Create(
	...
	void createAoPipeline()
	{
		...
		auto shaderStageCI = Shit::PipelineShaderStageCreateInfo{
			p->getStage(), p->getHandle(), p->getEntryName().data()};
		...
		for (int i = 0; i < count; ++i)
		{
			...
			aoImageViews.emplace_back(_device->Create(
		...
		aoPipelineInfo.pipeline = _device->Create(
			Shit::ComputePipelineCreateInfo{shaderStageCI, aoPipelineInfo.pipelineLayout});
	}
	void createShadingPipeline()
	{
		auto p = shadingPipelineInfo.shaders[0];
		p->load();
		auto shaderStageCI = Shit::PipelineShaderStageCreateInfo{p->getStage(), p->getHandle(), p->getEntryName().data()};
		...
		for (int i = 0; i < count; ++i)
		{
			auto pImage = shadingImages.emplace_back(_device->Create(
				Shit::ImageCreateInfo{
					...
					Shit::MemoryPropertyFlagBits::DEVICE_LOCAL_BIT,
					Shit::ImageLayout::GENERAL}));
			shadingImageViews.emplace_back(_device->Create(
				Shit::ImageViewCreateInfo{
					pImage,
					Shit::ImageViewType::TYPE_2D,
					Shit::Format::R8G8B8A8_UNORM,
		...
		shadingPipelineInfo.pipeline = _device->Create(
			Shit::ComputePipelineCreateInfo{shaderStageCI, shadingPipelineInfo.pipelineLayout});
	}
	...
	void updateDescriptorSets()
	{
		...
		std::vector<Shit::WriteDescriptorSet> writes;
		{
			...
			{
				imagesInfo[i * count + 0] = {depthSampler, gpassRenderTarget.imageViews[(i + 1) * gpassAttachmentCount - 1], Shit::ImageLayout::SHADER_READ_ONLY_OPTIMAL};
				imagesInfo[i * count + 1] = {posSampler, gpassRenderTarget.imageViews[i * gpassAttachmentCount + 0], Shit::ImageLayout::SHADER_READ_ONLY_OPTIMAL};
				imagesInfo[i * count + 2] = {sampler, gpassRenderTarget.imageViews[i * gpassAttachmentCount + 1], Shit::ImageLayout::SHADER_READ_ONLY_OPTIMAL};
				imagesInfo[i * count + 3] = {0, aoImageViews[i], Shit::ImageLayout::GENERAL};
				writes.emplace_back(
					aoPipelineInfo.descriptorSets[i],
					0,
					0,
					1,
					Shit::DescriptorType::COMBINED_IMAGE_SAMPLER,
					&imagesInfo[i * count + 0]);
				writes.emplace_back(
					aoPipelineInfo.descriptorSets[i],
					1,
					0,
		...
		}
		{
			...
			for (uint32_t i = 0; i < frameCount; ++i)
			{
				...
				{
					imagesInfo[i * count + j] = Shit::DescriptorImageInfo{
						sampler,
						gpassRenderTarget.imageViews[i * gpassAttachmentCount + j],
						Shit::ImageLayout::SHADER_READ_ONLY_OPTIMAL};

					writes.emplace_back(Shit::WriteDescriptorSet{
						shadingPipelineInfo.descriptorSets[i],
						j,
						0,
						1,
						Shit::DescriptorType::COMBINED_IMAGE_SAMPLER,
				...
				writes.emplace_back(
					shadingPipelineInfo.descriptorSets[i + frameCount],
					5,
					0,
					1,
					Shit::DescriptorType::COMBINED_IMAGE_SAMPLER,
					&imagesInfo[i * count + 5]);
				writes.emplace_back(
					shadingPipelineInfo.descriptorSets[i + 2 * frameCount],
					0,
					0,
					1,
					Shit::DescriptorType::STORAGE_IMAGE,
					&imagesInfo[i * count + 6]);
			}
			_device->UpdateDescriptorSets(writes);
			writes.clear();
		}
		{
			...
			for (uint32_t i = 0; i < frameCount; ++i)
			{
				...
				writes.emplace_back(
					postProcessPipelineInfo.descriptorSets[i],
					0,
					0,
					1,
					Shit::DescriptorType::COMBINED_IMAGE_SAMPLER,
	...
	void drawGameObject(Shit::CommandBuffer *cmdBuffer, GameObject *gameObject, Shit::PipelineLayout *pipelineLayout)
	{
	...
	void recordDefaultCommandBuffer() override
	{
		...
		{
			// bind camera
			auto cameraDescriptorSet = _editorCamera->getDescriptorSet(i);

			auto cmdBuffer = _defaultCommandBuffers[i];
			cmdBuffer->Reset();
			cmdBuffer->Begin({});
			...
			cmdBuffer->BeginRenderPass(
				Shit::BeginRenderPassInfo{
					gpassRenderTarget.renderPass,
					gpassRenderTarget.frameBuffers[i],
			...
			cmdBuffer->BindPipeline(Shit::BindPipelineInfo{
				Shit::PipelineBindPoint::GRAPHICS,
				gpassPipelineInfo.pipeline});

			for (auto &&e : _gameObjects)
				drawGameObject(cmdBuffer, e.get(), gpassPipelineInfo.pipelineLayout);
			...
			cmdBuffer->PushConstants({aoPipelineInfo.pipelineLayout,
									  Shit::ShaderStageFlagBits::COMPUTE_BIT,
									  0,
									  sizeof(AOPushConstant),
									  &aoPc});
			...
			cmdBuffer->GenerateMipmap({aoImages[i],
									   Shit::Filter::LINEAR,
									   Shit::ImageLayout::GENERAL,
									   Shit::ImageLayout::GENERAL});
			...
			cmdBuffer->BindDescriptorSets(Shit::BindDescriptorSetsInfo{
				Shit::PipelineBindPoint::COMPUTE,
				shadingPipelineInfo.pipelineLayout,
				0,
				1,
				&cameraDescriptorSet});
			...
			cmdBuffer->BindDescriptorSets(Shit::BindDescriptorSetsInfo{
				...
				3,
				1,
				&shadingPipelineInfo.descriptorSets[i + frameCount * 2]});

			cmdBuffer->PushConstants({shadingPipelineInfo.pipelineLayout,
									  Shit::ShaderStageFlagBits::COMPUTE_BIT,
									  0,
									  sizeof(ShadingPushConstant),
									  &shadingPc});
			...
			cmdBuffer->BindPipeline(Shit::BindPipelineInfo{
				Shit::PipelineBindPoint::GRAPHICS,
				postProcessPipelineInfo.pipeline});
	...
	void setupImGui()
	{
		...
		//_needUpdateDefaultCommandBuffer |= ImGui::Checkbox("enableBlur", &enableBlur);
		{
			static const char *items[]{
				"calc ao",
				"calc ao width multibounce",
				"render with ao",
				"texture ao",
				"no ao",
			};
	...
	void recreateGpass()
	{
		for (auto e : gpassRenderTarget.images)
			_device->Destroy(e);
		gpassRenderTarget.images.clear();
		for (auto e : gpassRenderTarget.imageViews)
			_device->Destroy(e);
		gpassRenderTarget.imageViews.clear();
		for (auto e : gpassRenderTarget.frameBuffers)
			_device->Destroy(e);
