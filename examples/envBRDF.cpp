...
public:
	void initRenderSystem()
	{
		...
		Shit::WindowCreateInfo windowCreateInfo{
			{Shit::WindowCreateFlagBits::INVISIBLE},
			__WFILE__,
	...
	void preparePipeline()
	{
		...
		auto descriptorPool = device->Create(Shit::DescriptorPoolCreateInfo{1, (uint32_t)poolSize.size(), poolSize.data()});
		...
		auto shader = device->Create(Shit::ShaderCreateInfo{g_shaderSourceLanguage, shaderSource.size(), shaderSource.data()});

		uint32_t constantIDs[] = {COSTANT_ID_COMPUTE_LOCAL_SIZE_X};
		uint32_t constantValues[] = {WIDTH};
		auto stageCI = Shit::PipelineShaderStageCreateInfo{
			Shit::ShaderStageFlagBits::COMPUTE_BIT, shader, "main",
	...
	{
		preparePipeline();
		auto envBRDFImage = device->Create(
		...
		auto envBRDFImageView = device->Create(
		...
		Shit::DescriptorImageInfo imagesInfo[]{
			{0, envBRDFImageView, Shit::ImageLayout::GENERAL}};
		...
		executeOneTimeCommands(
			device, [&](Shit::CommandBuffer *cmdBuffer)
			{
				...
				cmdBuffer->BindDescriptorSets({Shit::PipelineBindPoint::COMPUTE,
											   pipelineLayout,
											   0,
											   1,
											   &descriptorSet});
