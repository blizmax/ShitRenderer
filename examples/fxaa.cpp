...
public:
	...
	void createShadowRenderTargets()
	{
		...
		for (auto &&shadowRenderTarget : shadowRenderTargets)
		{
			...
			auto depthImageCI = Shit::ImageCreateInfo{
				...
				Shit::ImageLayout::SHADER_READ_ONLY_OPTIMAL};
			auto depthImageViewCI = Shit::ImageViewCreateInfo{
				nullptr,
			...
			shadowRenderTarget.frameBuffers.emplace_back(_device->Create(Shit::FramebufferCreateInfo{
	...
	void createShadowPipeline()
	{
		...
		shadowPipelineInfo.shaders = {
			static_cast<Shader *>(_shaderManager->create(
				buildShaderPath(_device, shadowVertShaderPath, _rendererVersion), ResourceManager::DEFAULT_LOADER_NAME, {{"stage", "VERT"}})),
			static_cast<Shader *>(_shaderManager->create(
				buildShaderPath(_device, shadowGeomShaderPath, _rendererVersion), ResourceManager::DEFAULT_LOADER_NAME, {{"stage", "GEOM"}})),
			static_cast<Shader *>(_shaderManager->create(
				buildShaderPath(_device, shadowFragShaderPath, _rendererVersion), ResourceManager::DEFAULT_LOADER_NAME, {{"stage", "FRAG"}})),
		};
		...
		shadowPipelineInfo.pipeline = _device->Create(Shit::GraphicsPipelineCreateInfo{
			...
			{},										  // PipelineTessellationStateCreateInfo
			{
				...
				isVK ? Shit::CullMode::FRONT : Shit::CullMode::BACK,
	...
	void createRenderPass() 
	{
		...
		renderTarget.renderPass = _device->Create(
			Shit::RenderPassCreateInfo{
				(uint32_t)attachmentDesc.size(),
				attachmentDesc.data(),
		...
		renderTarget.clearValues.assign(
			attachmentCount, clearColor);
	...
	void createRenderTarget()
	{
		...
		{
			// deferred render target
			auto pImage = renderTarget.images.emplace_back(_device->Create(
				Shit::ImageCreateInfo{
					...
					Shit::ImageUsageFlagBits::COLOR_ATTACHMENT_BIT |
						Shit::ImageUsageFlagBits::SAMPLED_BIT,
					Shit::MemoryPropertyFlagBits::DEVICE_LOCAL_BIT}));
			renderTarget.imageViews.emplace_back(_device->Create(
	...
	void preparePipelines()
	{
		...
		postProcessPipelineInfo.shaders = {
			static_cast<Shader *>(_shaderManager->create(
				buildShaderPath(_device, readVertShaderPath, _rendererVersion), ResourceManager::DEFAULT_LOADER_NAME, {{"stage", "VERT"}})),
			static_cast<Shader *>(_shaderManager->create(
				buildShaderPath(_device, postProcessFragShaderPath, _rendererVersion), ResourceManager::DEFAULT_LOADER_NAME, {{"stage", "FRAG"}}))};
		...
		Shit::PushConstantRange pcRange{
			Shit::ShaderStageFlagBits::FRAGMENT_BIT,
			0,
			0,
			sizeof(UBORead)};
		readPipelineInfo.pipelineLayout = _device->Create(
			Shit::PipelineLayoutCreateInfo{
				(uint32_t)descriptorSetLayouts.size(),
				descriptorSetLayouts.data(),
				1, &pcRange});
		...
		// postprocess pipeline descriptor sets
		{
			...
			Shit::PushConstantRange pcRange{
				...
				1,
				0,
				sizeof(PostProcessPC)};

			postProcessPipelineInfo.pipelineLayout = _device->Create(
				Shit::PipelineLayoutCreateInfo{1, &descriptorSetLayout, 1, &pcRange});
	...
	void updateDescriptorSets()
	{
		...
		for (uint32_t i = 0; i < getFramesInFlight(); ++i)
		{
			imagesInfo[i * attachmentCount] = {
				sampler, renderTarget.imageViews[i * attachmentCount], Shit::ImageLayout::SHADER_READ_ONLY_OPTIMAL};
			...
			for (uint32_t j = 1; j < 6; ++j)
			{
				imagesInfo[i * attachmentCount + j] = Shit::DescriptorImageInfo{
					0,
					renderTarget.imageViews[i * attachmentCount + j],
					Shit::ImageLayout::SHADER_READ_ONLY_OPTIMAL};

				writes.emplace_back(Shit::WriteDescriptorSet{
					readPipelineInfo.descriptorSets[i],
					j,
					0,
					1,
		...
		{
			imagesInfo2[i] = {
				sampler, shadowRenderTargets[i].imageViews[0],
				Shit::ImageLayout::SHADER_READ_ONLY_OPTIMAL};
			writes.emplace_back(Shit::WriteDescriptorSet{
	...
	void recordDefaultCommandBuffer() override
	{
		...
		for (int i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i)
		{
			...
			for (size_t j = 0; j < lights.size(); ++j)
			{
				...
				// bind camera
				auto descriptorSet = _editorCamera->getDescriptorSet(i);
				cmdBuffer->BindDescriptorSets(Shit::BindDescriptorSetsInfo{
					Shit::PipelineBindPoint::GRAPHICS,
					shadowPipelineInfo.pipelineLayout,
					1,
			...
			cmdBuffer->BindPipeline(Shit::BindPipelineInfo{
				Shit::PipelineBindPoint::GRAPHICS,
				writePipelineInfo.pipeline});

			for (auto &&e : _gameObjects)
				drawGameObject(cmdBuffer, e.get(), writePipelineInfo.pipelineLayout);

			cmdBuffer->NextSubpass(Shit::SubpassContents::INLINE);
			cmdBuffer->BindPipeline(Shit::BindPipelineInfo{
				Shit::PipelineBindPoint::GRAPHICS,
				readPipelineInfo.pipeline});
			...
			// bind camera
			descriptorSet = _editorCamera->getDescriptorSet(i);
			cmdBuffer->BindDescriptorSets(Shit::BindDescriptorSetsInfo{
				Shit::PipelineBindPoint::GRAPHICS,
				readPipelineInfo.pipelineLayout,
				1,
			...
			cmdBuffer->PushConstants({readPipelineInfo.pipelineLayout,
									  Shit::ShaderStageFlagBits::FRAGMENT_BIT,
									  0,
									  sizeof(UBORead),
									  &uboRead});
			...
			cmdBuffer->BindDescriptorSets(Shit::BindDescriptorSetsInfo{
				...
				1,
				&postProcessPipelineInfo.descriptorSets[i]});
			cmdBuffer->PushConstants({postProcessPipelineInfo.pipelineLayout,
									  Shit::ShaderStageFlagBits::FRAGMENT_BIT,
									  0,
									  sizeof(PostProcessPC),
									  &postProcessPC});
	...
	void setupImGui()
	{
		...
		_needUpdateDefaultCommandBuffer |= ImGui::InputFloat("fxaaThreasholdMin", &postProcessPC.fxaaThreasholdMin, 0.01);
	...
	{
		for (auto &&e : renderTarget.imageViews)
			_device->Destroy(e);
		for (auto &&e : renderTarget.images)
			_device->Destroy(e);
		for (auto &&e : renderTarget.frameBuffers)
			_device->Destroy(e);
