...
public:
	...
	void loadAssets()
	{
		...
		auto materialDatablock = (*sphere->childBegin())->getComponent<MeshRenderer>()->getPrimitiveRenderer(0)->getMaterialDataBlock();
		...
		materialDatablock = (*disk->childBegin())->getComponent<MeshRenderer>()->getPrimitiveRenderer(0)->getMaterialDataBlock();
		memcpy(materialDatablock->getUBOData()->emission, emission, sizeof(float) * 3);
		memset(materialDatablock->getUBOData()->diffuse, 0, sizeof(float) * 3);
		...
		materialDatablock = (*quad->childBegin())->getComponent<MeshRenderer>()->getPrimitiveRenderer(0)->getMaterialDataBlock();
		memcpy(materialDatablock->getUBOData()->emission, emission, sizeof(float) * 3);
		memset(materialDatablock->getUBOData()->diffuse, 0, sizeof(float) * 3);
	...
	void prepareShowLightPipeline()
	{
		...
		showLightPipelineInfo.shaders = {
			static_cast<Shader *>(_shaderManager->create(
				buildShaderPath(_device, vertShaderPath, _rendererVersion), ResourceManager::DEFAULT_LOADER_NAME, {{"stage", "VERT"}})),
			static_cast<Shader *>(_shaderManager->create(
				buildShaderPath(_device, showLightFragShaderPath, _rendererVersion), ResourceManager::DEFAULT_LOADER_NAME, {{"stage", "FRAG"}}))};
	...
	void recordDefaultCommandBuffer() override
	{
		...
		for (int i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i)
		{
			...
			cmdBuffer->BindPipeline(Shit::BindPipelineInfo{
				Shit::PipelineBindPoint::GRAPHICS,
				showLightPipelineInfo.pipeline});
