...
public:
	...
	void prepareDisplayPipeline()
	{
		...
		Shit::PushConstantRange ranges[]{
			{Shit::ShaderStageFlagBits::FRAGMENT_BIT, 1, 0, sizeof(DisplayPara)}};
	...
	{
		auto setLayout = dynamic_cast<Shit::GraphicsPipeline *>(displayPipelineInfo.pipeline)->GetCreateInfoPtr()->pLayout->GetCreateInfoPtr()->pSetLayouts[0];
		...
		Shit::DescriptorImageInfo imagesInfo[]{
			{0, inTexture->getImageView(), Shit::ImageLayout::GENERAL},
	...
	void prepareComputePipeline()
	{
		...
		auto descriptorSetLayout = _device->Create(Shit::DescriptorSetLayoutCreateInfo{std::size(bindings), &bindings[0]});

		auto pipelineLayout = _device->Create(Shit::PipelineLayoutCreateInfo{
			1, &descriptorSetLayout});

		compPipelineInfo.shaders = {
			static_cast<Shader *>(_shaderManager->create(
				buildShaderPath(_device, compShaderPath, _rendererVersion), ResourceManager::DEFAULT_LOADER_NAME, {{"stage", "COMP"}}))};
	...
	{
		auto setLayout = dynamic_cast<Shit::ComputePipeline *>(compPipelineInfo.pipeline)->GetCreateInfoPtr()->pLayout->GetCreateInfoPtr()->pSetLayouts[0];
		...
		Shit::WriteDescriptorSet writes[]{
			...
			{compDescriptorSet, 0, 0, 1, Shit::DescriptorType::STORAGE_IMAGE, &imagesInfo[0]},
	...
	void processImage()
	{
		executeOneTimeCommands(
			_device, [&](Shit::CommandBuffer *pCommandBuffer)
			{
				pCommandBuffer->BindPipeline(Shit::BindPipelineInfo{
					Shit::PipelineBindPoint::COMPUTE,
					compPipelineInfo.pipeline});
				pCommandBuffer->BindDescriptorSets(Shit::BindDescriptorSetsInfo{
					Shit::PipelineBindPoint::COMPUTE,
					dynamic_cast<Shit::ComputePipeline *>(compPipelineInfo.pipeline)->GetCreateInfoPtr()->pLayout,
					0,
					1,
					&compDescriptorSet});
	...
	void setupImGui()
	{
		...
		static bool showOriginImage = false;
		if (ImGui::Checkbox("origin image", &showOriginImage))
		{
			displayFlag = true;
			currentDescriptorSet = showOriginImage ? descriptorSetInTex : descriptorSetOutTex;
		}
		...
		if (compProcessPara.mode == 2)
		{
			...
			if (ImGui::Button("do filter"))
			{
				...
				case 0:
					...
					break;
				case 1:
					compKernel = (1 + k[0]) * FilterKernel() -
								 k[0] * FilterKernel(filterPattern[0], radius[0]);
					break;
				case 2:
