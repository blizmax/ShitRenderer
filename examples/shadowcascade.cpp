...
class LightController : public Behaviour
{
	...
	{
		getParentTransform()->pitch(glm::radians(-30.f));
	}
...
public:
	void loadAssets()
	{
		...
		for (; j < row; ++j)
		{
			for (i = 0; i < column; ++i)
			{
				...
				treeObj->getComponent<Transform>()->translate(
					{float(2 * i - column + 1) / 2 * interval,
					 1,
					 float(2 * j - row + 1) / 2 * interval});
				Model *pModel = tree;
				if (i % 2)
				{
					pModel = cylinder;
		...
		uboDisplayBuffer = _device->Create(Shit::BufferCreateInfo{
											   {},
											   sizeof(UBODisplay),
		...
		_editorCamera->getParentTransform()->addListener(std::bind(&Hello::cameraListener, this, std::placeholders::_1, std::placeholders::_2));
		_editorCamera->addListener(std::bind(&Hello::cameraListener, this, std::placeholders::_1, std::placeholders::_2));
	...
	void prepareShowTexPipeline()
	{
		...
		auto pipelineLayoutCI = Shit::PipelineLayoutCreateInfo{
		...
		//===============================
		showTexPipelineInfo.shaders = {
			static_cast<Shader *>(_shaderManager->create(
				buildShaderPath(_device, showTexVertShaderPath, _rendererVersion), ResourceManager::DEFAULT_LOADER_NAME, {{"stage", "VERT"}})),
			static_cast<Shader *>(_shaderManager->create(
				buildShaderPath(_device, showTexFragShaderPath, _rendererVersion), ResourceManager::DEFAULT_LOADER_NAME, {{"stage", "FRAG"}}))};
	...
	void preparePipeline()
	{
		...
		std::vector<Shit::DescriptorSetLayout *> setLayouts(MAX_FRAMES_IN_FLIGHT, descriptorSetLayout);
		pipelineInfo.descriptorSets.resize(MAX_FRAMES_IN_FLIGHT);
		...
		pipelineInfo.shaders = {
			static_cast<Shader *>(_shaderManager->create(
				buildShaderPath(_device, vertShaderPath, _rendererVersion), ResourceManager::DEFAULT_LOADER_NAME, {{"stage", "VERT"}})),
			static_cast<Shader *>(_shaderManager->create(
				buildShaderPath(_device, fragShaderPath, _rendererVersion), ResourceManager::DEFAULT_LOADER_NAME, {{"stage", "FRAG"}}))};
	...
	void writeDescriptorSets()
	{
		...
		Shit::DescriptorImageInfo imageInfo{
			getSampler("linear"), shadowRenderTarget.imageViews[0], Shit::ImageLayout::SHADER_READ_ONLY_OPTIMAL};

		std::vector<Shit::WriteDescriptorSet> writeSets;
		for (int i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i)
		{
			writeSets.emplace_back(
				Shit::WriteDescriptorSet{pipelineInfo.descriptorSets[i], 2, 0, 1, Shit::DescriptorType::UNIFORM_BUFFER, 0, &bufferInfo});
			writeSets.emplace_back(
				Shit::WriteDescriptorSet{pipelineInfo.descriptorSets[i], 8, 0, 1, Shit::DescriptorType::COMBINED_IMAGE_SAMPLER, &imageInfo});
		};
	...
	void writeShowTexDescriptorSets()
	{
		...
		std::vector<Shit::WriteDescriptorSet> writeSets{
			{showTexPipelineInfo.descriptorSets[0], 0, 0, 1,
	...
	void cameraListener(Component const *pComp, Component::Event event)
	{
		if (event != Component::Event::UPDATE)
			return;
		std::vector<glm::vec3> vertices;
		for (int i = 0; i < _editorCamera->getCascadeNum(); ++i)
		{
	...
	void recordDefaultCommandBuffer() override
	{
		...
		for (int i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i)
		{
			...
			cmdBuffer->BindDescriptorSets(Shit::BindDescriptorSetsInfo{
				Shit::PipelineBindPoint::GRAPHICS,
				shadowPipeline->GetCreateInfoPtr()->pLayout,
				1,
				1,
				&descriptorSet});
			...
			for (auto &&e : _gameObjects)
				drawGameObject(cmdBuffer, e.get(), shadowPipeline->GetCreateInfoPtr()->pLayout);
			...
			else if (viewMode == 1)
			{
				...
				cmdBuffer->PushConstants({pipeline->GetCreateInfoPtr()->pLayout,
	...
	void setupImGui()
	{
		...
		ImGui::Separator();
		{
			...
			if (viewMode == 0)
			{
				...
				updateUBOBuffer |= ImGui::SliderFloat("shadowFilterRadius", &uboDisplayData.shadowFilterRadius, 1.f, 10.f);
				if (updateUBOBuffer)
				{
			...
			if (viewMode == 1)
			{
				_needUpdateDefaultCommandBuffer |= ImGui::SliderInt("texLayer", &showTexVar.texLayer, 0, cascadeNum - 1);
			}
		}
		ImGui::End();
	}
