...
public:
	...
	void createDefaultRenderTarget() override
	{
		...
		_defaultRenderTarget.frameBuffers.resize(MAX_FRAMES_IN_FLIGHT);
	...
	void createMultiviewRenderTarget()
	{
		...
		if (_sampleCount != Shit::SampleCountFlagBits::BIT_1)
		{
			...
			attachmentDesc.emplace_back(
				...
				Shit::ImageLayout::UNDEFINED,
				Shit::ImageLayout::SHADER_READ_ONLY_OPTIMAL);
			multiviewRenderTarget.clearValues.emplace_back(
				Shit::ClearColorValueFloat{0.2, 0.2, 0.2, 1.});
		}
		Shit::AttachmentReference colorAttachment{0, Shit::ImageLayout::COLOR_ATTACHMENT_OPTIMAL};
		Shit::AttachmentReference depthAttachment{1, Shit::ImageLayout::DEPTH_STENCIL_ATTACHMENT_OPTIMAL};
		...
		multiviewRenderTarget.renderPass = _device->Create(
			Shit::RenderPassCreateInfo{
				(uint32_t)attachmentDesc.size(),
				attachmentDesc.data(),
				1,
		...
		multiviewRenderTarget.frameBuffers.resize(MAX_FRAMES_IN_FLIGHT);
		for (int i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i)
		{
	...
	void createMultiviewPipeline()
	{
		...
		Shit::DescriptorSetLayoutBinding descriptorSetLayoutBindings[]{
			{2, Shit::DescriptorType::UNIFORM_BUFFER, 1, Shit::ShaderStageFlagBits::VERTEX_BIT | Shit::ShaderStageFlagBits::FRAGMENT_BIT},
		};
		...
		auto pipelineLayout = _device->Create(
			Shit::PipelineLayoutCreateInfo{
				std::size(descriptorSetLayouts), descriptorSetLayouts});

		//===============================
		multiviewPipelineInfo.shaders = {
			static_cast<Shader *>(_shaderManager->create(
				buildShaderPath(_device, multiviewVertShaderPath, _rendererVersion),
				ResourceManager::DEFAULT_LOADER_NAME,
				{{"stage", "VERT"}})),
			static_cast<Shader *>(_shaderManager->create(
	...
	void createPresentPipeline()
	{
		...
		presentPipelineInfo.descriptorSets.resize(MAX_FRAMES_IN_FLIGHT);
	...
	void createCamera()
	{
		...
		camera->getParentTransform()
			->translate({0.1, 0, 0})
			->yaw(glm::radians(10.f));

		camera->addListener(std::bind(&Hello::updateCamera, this, std::placeholders::_1, std::placeholders::_2));
	...
	void updateCamera(Component const *, Camera::Event event)
	{
		if (event == Camera::Event::UPDATE)
		{
			UBOCamera a[]{
				...
				{camera->getViewMatrix(),
				 ...
				 camera->getParentTransform()->getGlobalPosition()},
			};
			void *data;
	...
	{
		std::vector<Shit::DescriptorImageInfo> imageInfos(MAX_FRAMES_IN_FLIGHT);
		std::vector<Shit::WriteDescriptorSet> writes(MAX_FRAMES_IN_FLIGHT);
		...
		for (int i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i)
		{
			...
			writes[i] = {
				presentPipelineInfo.descriptorSets[i],
				0,
				0,
				1,
		...
		for (int i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i)
		{
			...
			cmdBuffer->BeginRenderPass(
				Shit::BeginRenderPassInfo{
					multiviewRenderTarget.renderPass,
					multiviewRenderTarget.frameBuffers[i],
					{{}, multiviewRenderTarget.frameBuffers[i]->GetCreateInfoPtr()->extent},
			...
			cmdBuffer->BindPipeline(Shit::BindPipelineInfo{
				Shit::PipelineBindPoint::GRAPHICS,
				multiviewPipelineInfo.pipeline});
			...
			cmdBuffer->BeginRenderPass(
				Shit::BeginRenderPassInfo{
					...
					{{}, _defaultRenderTarget.frameBuffers[i]->GetCreateInfoPtr()->extent},
