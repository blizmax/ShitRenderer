...
public:
	void loadAssets()
	{
		...
		// auto materialId = (*model->meshBegin())->primitives[0]->faceMaterialIds[0];
		// if (materialId == -1)
		//	pMaterialDataBlock = _materialDataBlockManager->getMaterialDataBlockById();
		// else
		//	pMaterialDataBlock = _materialDataBlockManager->getMaterialDataBlockById(materialId);
		...
		// envbrdf
		// auto paras = ParameterMap{{"request_component_num", "2"}};
	...
	void prepareEq2CubePipeline()
	{
		...
		auto descriptorSetLayout = _device->Create(
			Shit::DescriptorSetLayoutCreateInfo{std::size(bindings), bindings});
		...
		//==============
		eq2cubePipelineInfo.shaders = {
			static_cast<Shader *>(_shaderManager->create(
				buildShaderPath(_device, eq2CubeCompShaderName, _rendererVersion),
		...
		eq2cubePipelineInfo.pipeline = _device->Create(
			Shit::ComputePipelineCreateInfo{shaderStageCI, pipelineLayout});
	}
	void prepareSkyboxPipeline()
	{
		...
		skyboxPipelineInfo.shaders = {
			static_cast<Shader *>(_shaderManager->create(
				buildShaderPath(_device, skyboxVertShaderPath, _rendererVersion),
				ResourceManager::DEFAULT_LOADER_NAME, {{"stage", "VERT"}})),
			static_cast<Shader *>(_shaderManager->create(
				buildShaderPath(_device, skyboxFragShaderPath, _rendererVersion),
	...
	void preparePrefilteredEnvPipeline()
	{
		...
		//==============
		prefilteredEnvPipelineInfo.shaders = {
			static_cast<Shader *>(_shaderManager->create(
				buildShaderPath(_device, prefilteredEnvCompShaderName, _rendererVersion),
				ResourceManager::DEFAULT_LOADER_NAME, {{"stage", "COMP"}}))};

		prefilteredEnvPipelineInfo.shaders[0]->load();
		Shit::PipelineShaderStageCreateInfo shaderStageCI{
			prefilteredEnvPipelineInfo.shaders[0]->getStage(),
			prefilteredEnvPipelineInfo.shaders[0]->getHandle(),
			prefilteredEnvPipelineInfo.shaders[0]->getEntryName().data()};

		prefilteredEnvPipelineInfo.pipeline = _device->Create(
			Shit::ComputePipelineCreateInfo{shaderStageCI, pipelineLayout});
	}
	...
	void updateCubeTex(Texture *eqTex, Texture *cubeTex_)
	{
		...
		Shit::DescriptorImageInfo imageInfos[]{
			{sampler,
			 ...
			 Shit::ImageLayout::SHADER_READ_ONLY_OPTIMAL},
			{0,
			 cubeTex_->getImageView(),
			 Shit::ImageLayout::GENERAL},
		};
		Shit::WriteDescriptorSet writeSets[]{
			{eq2cubePipelineInfo.descriptorSets[0],
			 0,
			 0,
			 1,
			...
			[&](Shit::CommandBuffer *cmdBuffer)
			{
				Shit::ImageMemoryBarrier barrier{
					...
					{Shit::ImageAspectFlagBits::COLOR_BIT,
					 0,
					 cubeTex->getImage()->GetCreateInfoPtr()->mipLevels,
					 0,
	...
	void updatePrefilteredEnvTex(Texture *eqTex, Texture *cubeTex_)
	{
		...
		{
			cubeTexImageViews[i] = _device->Create(Shit::ImageViewCreateInfo{
				cubeTex_->getImage(),
				Shit::ImageViewType::TYPE_CUBE,
				cubeTex_->getImage()->GetCreateInfoPtr()->format,
				{},
				{Shit::ImageAspectFlagBits::COLOR_BIT, i, 1, 0, cubeTex_->getImage()->GetCreateInfoPtr()->arrayLayers}});
	...
	void writeSkyboDescriptorSet()
	{
		...
		Shit::DescriptorImageInfo imageInfo{
			getSampler("linear"),
			cubeTex->getImageView(),
			Shit::ImageLayout::SHADER_READ_ONLY_OPTIMAL};
		Shit::WriteDescriptorSet writes[]{
			{skyboxPipelineInfo.descriptorSets[0],
			 0,
			 0,
			 1,
			 Shit::DescriptorType::COMBINED_IMAGE_SAMPLER,
	...
	void recordDefaultCommandBuffer() override
	{
		...
		for (int i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i)
		{
			...
			// bind camera
			auto tempDescriptorSet = _editorCamera->getDescriptorSet(i);
			...
			cmdBuffer->BindPipeline(
				{Shit::PipelineBindPoint::GRAPHICS, pipelineInfo.pipeline});
	...
	void setupImGui()
	{
		...
		flag |= ImGui::Checkbox("manual basecolor", (bool *)&uboOtherData.useManualBaseColor);
		...
		static std::vector<std::pair<std::string, glm::vec3>> baseColors{
			...
			{"Platinum ", {0.673, 0.637, 0.585}},
			{"Copper ", {0.955, 0.638, 0.538}},
			{"Palladium ", {0.733, 0.697, 0.652}},
			{"Mercury ", {0.781, 0.780, 0.778}},
			{"Brass C260) ", {0.910, 0.778, 0.423}},
			{"Zinc ", {0.664, 0.824, 0.850}},
			{"Gold ", {1.000, 0.782, 0.344}},
			{"Gold2 ", {0.604, 0.440, 0.012}},
		...
		flag |= ImGui::ColorEdit3("baseColor", (float *)&uboOtherData.baseColor, ImGuiColorEditFlags_NoLabel | misc_flags);
