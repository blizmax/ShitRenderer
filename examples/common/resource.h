...
public:
	...
	void addListener(Shit::Slot<void()> slot)
	{
		_signal.Connect(slot);
	}
	void removeListener(Shit::Slot<void()> slot)
	{
...
protected:
	...
	virtual Resource *createImpl(
		...
		ParameterMap const &parameters) = 0;
...
public:
	...
	Resource *create(std::string_view path,
					 std::string_view loaderName = DEFAULT_LOADER_NAME,
	...
	Resource *createOrRetrieve(std::string_view path,
							   std::string_view loaderName = DEFAULT_LOADER_NAME,
							   ParameterMap const &parameters = {})
	{
		if (auto ret = getResourceByName(path))
			return ret;
	...
	Resource *getResourceByName(std::string_view name) const
	{
		...
		if (it != _resourcesNameMap.end())
			return it->second;
		return nullptr;
	}
	Resource *getResourceById(uint32_t id) const
	{
		return _resources.at(id).get();
	}
	void removeResourceById(uint32_t id)
	{
		_resources.erase(id);
	}
	...
	//template <typename Loader_T, typename std::enable_if_t<std::is_base_of_v<ResourceLoader, Loader_T>, bool> = true>
