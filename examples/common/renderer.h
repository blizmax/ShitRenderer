...
public:
	...
	PrimitiveRenderer *getPrimitiveRenderer(size_t index) const
	{
		return _renderers.at(index).get();
	}
	...
	void draw(Shit::CommandBuffer *cmdBuffer,
			  Shit::PipelineLayout *pipelineLayout = nullptr,
...
public:
	...
	size_t getRenderersCount() const { return _renderers.size(); }
	PrimitivePointRenderer *getPrimitiveRenderer(size_t index) const
	{
		return _renderers.at(index).get();
	}
