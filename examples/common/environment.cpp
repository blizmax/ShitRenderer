...
Environment::Environment(Image const *pEqImage)
...
void Environment::createRenderPass(Shit::Format format)
{
	...
	_renderPass = g_App->getDevice()->Create(
...
void Environment::createEq2CubePipeline()
{
	...
	auto descriptorSetLayout = g_App->getDevice()->Create(
		Shit::DescriptorSetLayoutCreateInfo{std::size(bindings), &bindings[0]});
	...
	_eq2cubePipelineInfo.pipelineLayout = g_App->getDevice()->Create(
		Shit::PipelineLayoutCreateInfo{1, &descriptorSetLayout});

	//==============
	_eq2cubePipelineInfo.shaders = {
...
void Environment::createPrefilteredEnvPipelineInfo()
{
	...
	g_App->getDescriptorPool()->Allocate(
		{(uint32_t)setLayouts.size(), setLayouts.data()}, _prefilteredEnvPipelineInfo.descriptorSets.data());
	...
	//==============
	_prefilteredEnvPipelineInfo.shaders = {
		static_cast<Shader *>(g_App->getShaderManager()->create(
...
void Environment::generateSkyBox()
{
	...
	executeOneTimeCommands(
		...
		[&](Shit::CommandBuffer *cmdBuffer)
		{
			...
			//						   Shit::ImageLayout::GENERAL, Shit::ImageLayout::SHADER_READ_ONLY_OPTIMAL});
		});
}
void Environment::generateReflectionTex()
{
	...
	std::vector<Shit::DescriptorImageInfo> imageInfos{
		{sampler,
		 _envEquirectangularTex->getImageView(),
		 Shit::ImageLayout::SHADER_READ_ONLY_OPTIMAL},
		{}};
	...
	executeOneTimeCommands(
		...
		[&](Shit::CommandBuffer *cmdBuffer)
		{
			cmdBuffer->BindPipeline(Shit::BindPipelineInfo{
				Shit::PipelineBindPoint::COMPUTE,
				_prefilteredEnvPipelineInfo.pipeline});
...
void Environment::draw(Shit::CommandBuffer *cmdBuffer,
					   Shit::DescriptorSet *cameraDescriptorSet)
{
	cmdBuffer->BindPipeline(Shit::BindPipelineInfo{
		Shit::PipelineBindPoint::GRAPHICS,
		_skyboxPipelineInfo.pipeline});
