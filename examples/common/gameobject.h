...
public:
	...
	void addSignalListener(Shit::Slot<void(GameObject::Event)> slot)
	{
		_signal.Connect(slot);
	}
	void removeSignalListener(Shit::Slot<void(GameObject::Event)> slot)
	{
		_signal.Disconnect(slot);
	}

protected:
	Shit::Signal<void(GameObject::Event)> _signal;
};
