...
void ModelLoaderObj::load(Resource *resource)
{
	...
	static bool flipY = (g_App->getRendererVersion() & Shit::RendererVersion::TypeBitmask) == Shit::RendererVersion::VULKAN;
	...
	for (auto &&shape : shapes)
	{
		...
		for (size_t f = 0, lenf = shape.mesh.num_face_vertices.size(); f < lenf; ++f)
		{
			...
			for (unsigned char v = 0; v < fv; ++v, ++indexOffset)
			{
				...
				{
					trianglePrimitive->indices.emplace_back(
						vertexIndexMap[tempVertex] = (uint32_t)trianglePrimitive->positions.size());
		...
		if (dstModel->isEnableCalcDistanceAlongNormal())
			newMesh->calculateDistanceAlongNormal();
	}
}
Model::~Model()
{
...
{
	registerResourceLoader<ModelLoaderObj>(DEFAULT_LOADER_NAME);
}
Resource *ModelManager::createImpl(
