...
Texture::Texture(Shit::ImageCreateInfo const &imageCI, Shit::ImageViewType imageViewType, void *data)
{
	_image = g_App->getDevice()->Create(
		imageCI, Shit::ImageAspectFlagBits::COLOR_BIT, data);
	...
	_imageView = g_App->getDevice()->Create(
		Shit::ImageViewCreateInfo{
			...
			{Shit::ImageAspectFlagBits::COLOR_BIT,
			 0, _image->GetCreateInfoPtr()->mipLevels,
			 0, _image->GetCreateInfoPtr()->arrayLayers}});
}
Texture::~Texture()
{
...
{
	if (_status != Status::CREATED || !_imageSrc)
		return;
...
//						  imageViewCreateInfo, g_App->getDevice()->Create(imageViewCreateInfo))
//		.first->second;
//}
...
Texture *TextureManager::createTexture(Image const *image,
									   Shit::ImageUsageFlagBits usage,
									   Shit::ImageLayout imageLayout)
{
...
Texture *TextureManager::createOrRetrieveTexture(
	Image const *image, Shit::ImageUsageFlagBits usage, Shit::ImageLayout imageLayout)
{
	if (auto ret = getTexture(image->getName()))
		return ret;
...
Texture *TextureManager::getTexture(std::string_view imageName)
{
	...
	if (it != _texturesNameMap.end())
		return it->second;
	return nullptr;
}
Texture *TextureManager::createTexture(
	Shit::ImageCreateInfo const &imageCI, Shit::ImageViewType imageViewType, void *data)
{
