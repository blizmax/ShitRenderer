...
void MaterialDataBlock::prepare()
{
	...
	_textures.resize(
		descriptorSetLayout->GetCreateInfoPtr()->pDescriptorSetLayoutBindings[0].descriptorCount,
		g_App->getTextureManager()->getWhiteTexture());

	g_App->getDescriptorPool()->Allocate(
		Shit::DescriptorSetAllocateInfo{1, &descriptorSetLayout}, &_descriptorSet);
...
void MaterialDataBlock::setTexture(int &index, std::string_view imagePath, bool isSRGB)
{
	...
	auto pImageSrc = static_cast<Image *>(g_App->getImageManager()->createOrRetrieve(
		imagePath, ResourceManager::DEFAULT_LOADER_NAME, params));
	pImageSrc->load();
...
void MaterialDataBlock::setTextureRotateAngle(float rad)
{
	_uboMaterial.rotateAngle = rad;
}
//=============================
