...
bool AABB_intersect(
	glm::vec3 const &pmin_A,
	glm::vec3 const &pmax_A,
	glm::vec3 const &pmin_B,
	glm::vec3 const &pmax_B)
{
	for (int i = 0; i < 3; ++i)
	{
		if (pmin_A[i] > pmax_B[i] || pmin_B[i] > pmax_A[i])
			return false;
	}
	return true;
}
...
{
	if (extent == Extent::INFINITE || aabbBox.extent == Extent::NONE)
		return;
	else if (extent == Extent::NONE)
		*this = aabbBox;
	else if (aabbBox.extent == Extent::INFINITE)
