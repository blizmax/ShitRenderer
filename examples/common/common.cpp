...
void executeOneTimeCommands(Shit::Device *pDevice, const std::function<void(Shit::CommandBuffer *)> &func)
{
	static auto queueFamilyProperty = pDevice->GetQueueFamilyProperty(
	...
	static auto queue = pDevice->GetQueue(queueFamilyProperty->index, 0);
	static auto pFence = pDevice->Create(Shit::FenceCreateInfo{});
	pFence->Reset();
	static auto commandPool = pDevice->Create(Shit::CommandPoolCreateInfo{
...
void *loadImage(const char *imagePath, int &width, int &height, int &components, int request_components, int &componentSize)
{
	void *ret{};
...
void saveImage(const char *imagePath, int width, int height, int component, const void *data, bool hdr)
{
	...
	if (!ret)
		LOG("failed to save image", ret);
}
void freeImage(void *pData)
{
	stbi_image_free(pData);
}
std::tuple<shaderc_target_env, shaderc_env_version, shaderc_spirv_version>
getTargetEnvironment(Shit::RendererVersion rendererVersion)
{
...
std::string compileGlslToSpirv(std::string_view glslcode, Shit::ShaderStageFlagBits stage, Shit::RendererVersion renderVersion)
{
	...
	auto retCode = compiler.CompileGlslToSpv(glslcode.data(), glslcode.size(), getStage(stage), "shader.glsl", compileOptions);
...
std::string buildShaderPathGlsl(Shit::Device *pDevice, const char *shaderName, Shit::RendererVersion renderVersion)
{
	...
	{
		flag = false;
		g_shaderSourceLanguage = chooseShaderShourceLanguage({Shit::ShadingLanguage::SPIRV, Shit::ShadingLanguage::GLSL}, pDevice);
	}
...
Shit::WindowPixelFormat chooseSwapchainFormat(const std::vector<Shit::WindowPixelFormat> &candidates, Shit::Device *pDevice, Shit::Window *pWindow)
{
...
Shit::PresentMode choosePresentMode(const std::vector<Shit::PresentMode> &candidates, Shit::Device *pDevice, Shit::Window *window)
{
...
Shit::ShadingLanguage chooseShaderShourceLanguage(const std::vector<Shit::ShadingLanguage> &candidates, Shit::Device *pDevice)
{
...
void saveImage(const char *dstPath, Shit::Device *pDevice, Shit::Image *pImage, Shit::ImageLayout imageLayout)
{
	...
	// if constexpr (0 && static_cast<bool>(pImage->GetCreateInfoPtr()->memoryPropertyFlags & Shit::MemoryPropertyFlagBits::HOST_VISIBLE_BIT))
	//{
	//	void *data;
	//	pImage->MapMemory(0, size, &data);
	//	//swizzleColor
	//	if (
	//		pImage->GetCreateInfoPtr()->format == Shit::ShitFormat::B8G8R8A8_SRGB ||
	//		pImage->GetCreateInfoPtr()->format == Shit::ShitFormat::B8G8R8A8_UNORM)
	//	{
	...
	//	pImage->UnMapMemory();
	//	if (!static_cast<bool>(pImage->GetCreateInfoPtr()->memoryPropertyFlags & Shit::MemoryPropertyFlagBits::HOST_COHERENT_BIT))
	//	{
	...
	// else
	{
		...
		executeOneTimeCommands(
			pDevice, [&](Shit::CommandBuffer *cmdBuffer)
			{
				for (uint32_t i = 0; i < levels; ++i)
				{
				...
				for (uint32_t i = 0; i < levels; ++i,
							  ...
							  tempDepth = (std::max)(tempDepth / 2, 1u))
				{
					Shit::BufferImageCopy copyRegion{0, 0, 0, {Shit::ImageAspectFlagBits::COLOR_BIT, i, 0, layers}, {}, {tempWidth, tempHeight, tempDepth}};
					cmdBuffer->CopyImageToBuffer(Shit::CopyImageToBufferInfo{
						pImage,
						dst[i],
						1,
						&copyRegion});
		...
		pDevice->ExecuteOneTimeCommands(
			[&](Shit::CommandBuffer *cmdBuffer)
			{
				Shit::ImageMemoryBarrier pImageBarrier0{
					Shit::AccessFlagBits::MEMORY_READ_BIT,
					Shit::AccessFlagBits::TRANSFER_READ_BIT,
					imageLayout,
					Shit::ImageLayout::TRANSFER_SRC_OPTIMAL,
				...
				cmdBuffer->CopyImage(CopyImageInfo{
					...
					copyRegions.data()});
				Shit::ImageMemoryBarrier pImageBarrier1{
					Shit::AccessFlagBits::TRANSFER_READ_BIT,
					Shit::AccessFlagBits::MEMORY_READ_BIT,
					Shit::ImageLayout::TRANSFER_SRC_OPTIMAL,
					...
					pImage,
					Shit::ImageSubresourceRange{0, levels, 0, layers}};
				Shit::ImageMemoryBarrier dstImageBarrier1{
					Shit::AccessFlagBits::TRANSFER_WRITE_BIT,
					Shit::AccessFlagBits::MEMORY_READ_BIT,
					Shit::ImageLayout::TRANSFER_DST_OPTIMAL,
					  ...
					  tempDepth = (std::max)(tempDepth / 2, 1u))
		{
			...
			// swizzleColor
			if (
				pImage->GetCreateInfoPtr()->format == Shit::Format::B8G8R8A8_SRGB ||
			...
			if (dataType == Shit::DataType::FLOAT)
				...
				ss += ".jpg";
			saveImage(ss.c_str(), tempWidth, tempHeight * tempDepth * layers, component, data, dataType == Shit::DataType::FLOAT);
			dst[j]->UnMapMemory();
		}
		for (auto &&e : dst)
		{
...
void generateIrradianceMap(
	Shit::Device *pDevice,
	Shit::Image *pSrcImage2D,
	Shit::ImageLayout srcInitialLayout,
	Shit::ImageLayout srcFinalLayout,
	Shit::Image *pDstImageCube,
	Shit::ImageLayout dstInitialLayout,
	Shit::ImageLayout dstFinalLayout)
{
	...
	Shit::ImageView *pDstImageViewCube = pDevice->Create(Shit::ImageViewCreateInfo{
		pDstImageCube,
	...
	std::vector<Shit::DescriptorSetLayoutBinding> cubemapComputeBindings{
		Shit::DescriptorSetLayoutBinding{0, Shit::DescriptorType::COMBINED_IMAGE_SAMPLER, 1, Shit::ShaderStageFlagBits::COMPUTE_BIT},
		Shit::DescriptorSetLayoutBinding{1, Shit::DescriptorType::STORAGE_IMAGE, 1, Shit::ShaderStageFlagBits::COMPUTE_BIT},
	};
	std::vector<Shit::DescriptorSetLayout *> setLayouts = {
		pDevice->Create(Shit::DescriptorSetLayoutCreateInfo{(uint32_t)cubemapComputeBindings.size(), cubemapComputeBindings.data()}),
	};
	// create pipeline layout
	Shit::PipelineLayout *pipelineLayoutGenerateCubemap = pDevice->Create(Shit::PipelineLayoutCreateInfo{1, &setLayouts[0]});
	...
	// auto compSourceSpv = compileGlslToSpirv(compSource, Shit::ShaderStageFlagBits::COMPUTE_BIT, g_RendererVersion);
	Shit::Shader *compShaderGenerateCubemap = pDevice->Create(Shit::ShaderCreateInfo{g_shaderSourceLanguage, compSource.size(), compSource.data()});
		...
		pDevice, [&](Shit::CommandBuffer *cmdBuffer)
		{
			...
			if (srcInitialLayout != Shit::ImageLayout::SHADER_READ_ONLY_OPTIMAL)
			{
				imageBarriers.emplace_back(Shit::ImageMemoryBarrier{
					...
					ST_QUEUE_FAMILY_IGNORED,
					pSrcImage2D,
					Shit::ImageSubresourceRange{Shit::ImageAspectFlagBits::COLOR_BIT, 0, pSrcImage2D->GetCreateInfoPtr()->mipLevels, 0, 6}});
			}
			...
			if (dstFinalLayout != dstInitialLayout && pDstImageCube->GetCreateInfoPtr()->mipLevels > 1)
			{
				imageBarriers.emplace_back(Shit::ImageMemoryBarrier{
					...
					ST_QUEUE_FAMILY_IGNORED,
					pDstImageCube,
					Shit::ImageSubresourceRange{Shit::ImageAspectFlagBits::COLOR_BIT, 1, pDstImageCube->GetCreateInfoPtr()->mipLevels - 1, 0, 6}});
			}
			//change image layout to general
			cmdBuffer->PipelineBarrier(Shit::PipelineBarrierInfo{
				Shit::PipelineStageFlagBits::COMPUTE_SHADER_BIT,
	...
	Shit::ImageLayout dstFinalLayout)
{
	...
	Shit::ImageView *pDstImageViewCube = pDevice->Create(Shit::ImageViewCreateInfo{
		...
		{Shit::ImageAspectFlagBits::COLOR_BIT, 0, 1, 0, 6}});
	// prepare llm image
	Shit::Image *llmImage = pDevice->Create(
		Shit::ImageCreateInfo{
			{},
			...
			Shit::ImageLayout::GENERAL});
	Shit::ImageView *llmImageView = pDevice->Create(
	...
	std::vector<Shit::DescriptorSetLayoutBinding> shBindinds{
		Shit::DescriptorSetLayoutBinding{0, Shit::DescriptorType::STORAGE_IMAGE, 1, Shit::ShaderStageFlagBits::COMPUTE_BIT},
		Shit::DescriptorSetLayoutBinding{1, Shit::DescriptorType::STORAGE_IMAGE, 1, Shit::ShaderStageFlagBits::COMPUTE_BIT},
	};
	std::vector<Shit::DescriptorSetLayout *> setLayouts = {
	...
	// auto llmSource = compileGlslToSpirv(readFile(llmShaderPath.c_str()), Shit::ShaderStageFlagBits::COMPUTE_BIT, g_RendererVersion);
	// auto shSource = compileGlslToSpirv(readFile(shShaderPath.c_str()), Shit::ShaderStageFlagBits::COMPUTE_BIT, g_RendererVersion);
	Shit::Shader *llmShader = pDevice->Create(Shit::ShaderCreateInfo{g_shaderSourceLanguage, llmSource.size(), llmSource.data()});
	Shit::Shader *shShader = pDevice->Create(Shit::ShaderCreateInfo{g_shaderSourceLanguage, shSource.size(), shSource.data()});
	...
	Shit::Pipeline *shPipeline = pDevice->Create(Shit::ComputePipelineCreateInfo{
		Shit::PipelineShaderStageCreateInfo{
			Shit::PipelineShaderStageCreateInfo{
				...
				shShader,
				"main",
				{1, constantIDs, constantValues}},
		},
		shPipelineLayout});

	executeOneTimeCommands(
		pDevice, [&](Shit::CommandBuffer *pCommandBuffer)
		{
			...
			pCommandBuffer->PipelineBarrier(Shit::PipelineBarrierInfo{
				Shit::PipelineStageFlagBits::TOP_OF_PIPE_BIT,
				Shit::PipelineStageFlagBits::COMPUTE_SHADER_BIT,
				{},
				0,
			...
			pCommandBuffer->BindPipeline(Shit::BindPipelineInfo{
				Shit::PipelineBindPoint::COMPUTE,
				llmPipeline});
			...
			if (srcInitialLayout != srcFinalLayout)
			{
				barriers.emplace_back(
					Shit::ImageMemoryBarrier{
						...
						{Shit::ImageAspectFlagBits::COLOR_BIT, 1, pSrcImageView2D->GetCreateInfoPtr()->pImage->GetCreateInfoPtr()->mipLevels - 1, 0, 1}});
			}
			if (dstFinalLayout != Shit::ImageLayout::GENERAL)
			{
				barriers.emplace_back(
...
void generateImage2D(
	Shit::Device *pDevice,
	const char *spirvShaderName,
	Shit::Image *pDstImage2D,
	Shit::ImageLayout srcLayout,
	Shit::ImageLayout dstLayout)
{
	Shit::ImageView *pOutImageView = pDevice->Create(
		Shit::ImageViewCreateInfo{
			pDstImage2D,
			Shit::ImageViewType::TYPE_2D,
	...
	auto descriptorSetLayout = pDevice->Create(Shit::DescriptorSetLayoutCreateInfo{(uint32_t)bindings.size(), bindings.data()});
	std::vector<Shit::DescriptorPoolSize> poolSize{
		{Shit::DescriptorType::STORAGE_IMAGE, 1},
	};
	auto descriptorPool = pDevice->Create(Shit::DescriptorPoolCreateInfo{1, (uint32_t)poolSize.size(), poolSize.data()});
	...
	// auto shaderSourceSpv = compileGlslToSpirv(shaderSource, Shit::ShaderStageFlagBits::COMPUTE_BIT, g_RendererVersion);
	auto shader = pDevice->Create(Shit::ShaderCreateInfo{g_shaderSourceLanguage, shaderSource.size(), shaderSource.data()});
	...
	executeOneTimeCommands(
		pDevice,
		[&](Shit::CommandBuffer *pCommandBuffer)
		{
...
void generateEquirectangularProceduralSkybox(
	Shit::Device *pDevice,
	...
	Shit::ImageLayout dstLayout)
{
	...
	if (!pMoleculeOpticalDepthImageView)
	{
		Shit::Image *pMoleculeOpticalDepthImage = pDevice->Create(
			Shit::ImageCreateInfo{
				...
				Shit::MemoryPropertyFlagBits::DEVICE_LOCAL_BIT,
				Shit::ImageLayout::UNDEFINED});
		generateOpticalDepthImage(pDevice, 8, pMoleculeOpticalDepthImage, Shit::ImageLayout::UNDEFINED, Shit::ImageLayout::SHADER_READ_ONLY_OPTIMAL);
		pMoleculeOpticalDepthImageView = pDevice->Create(
			Shit::ImageViewCreateInfo{
				pMoleculeOpticalDepthImage,
				Shit::ImageViewType::TYPE_2D,
		...
		Shit::Image *pAerosolsOpticalDepthImage = pDevice->Create(
			Shit::ImageCreateInfo{
				...
				Shit::MemoryPropertyFlagBits::DEVICE_LOCAL_BIT,
				Shit::ImageLayout::UNDEFINED});
		generateOpticalDepthImage(pDevice, 1.2, pAerosolsOpticalDepthImage, Shit::ImageLayout::UNDEFINED, Shit::ImageLayout::SHADER_READ_ONLY_OPTIMAL);
		pAerosolsOpticalDepthImageView = pDevice->Create(
			Shit::ImageViewCreateInfo{
				pAerosolsOpticalDepthImage,
				Shit::ImageViewType::TYPE_2D,
				pAerosolsOpticalDepthImage->GetCreateInfoPtr()->format,
				{},
				{Shit::ImageAspectFlagBits::COLOR_BIT, 0, 1, 0, 1}});
	}
	// takeScreenshot(pDevice, pMoleculeOpticalDepthImage, Shit::ImageLayout::SHADER_READ_ONLY_OPTIMAL);
	// std::this_thread::sleep_for(std::chrono::seconds(1));
	// takeScreenshot(pDevice, pAerosolsOpticalDepthImage, Shit::ImageLayout::SHADER_READ_ONLY_OPTIMAL);
	// std::this_thread::sleep_for(std::chrono::seconds(1));

	//===============
	Shit::ImageView *pDstImageView = pDevice->Create(
		Shit::ImageViewCreateInfo{
			pDstImage2D,
			Shit::ImageViewType::TYPE_2D,
			pDstImage2D->GetCreateInfoPtr()->format,
	...
	std::vector<Shit::DescriptorSetLayoutBinding> cubemapComputeBindings{
		Shit::DescriptorSetLayoutBinding{0, Shit::DescriptorType::COMBINED_IMAGE_SAMPLER, 1, Shit::ShaderStageFlagBits::COMPUTE_BIT},
		Shit::DescriptorSetLayoutBinding{1, Shit::DescriptorType::COMBINED_IMAGE_SAMPLER, 1, Shit::ShaderStageFlagBits::COMPUTE_BIT},
		Shit::DescriptorSetLayoutBinding{2, Shit::DescriptorType::STORAGE_IMAGE, 1, Shit::ShaderStageFlagBits::COMPUTE_BIT},
		Shit::DescriptorSetLayoutBinding{3, Shit::DescriptorType::UNIFORM_BUFFER, 1, Shit::ShaderStageFlagBits::COMPUTE_BIT},
	};
	...
	auto shaderSource = readFile(shaderPath.c_str());
	// auto shaderSource = compileGlslToSpirv(readFile(shaderPath.c_str()), Shit::ShaderStageFlagBits::COMPUTE_BIT, g_RendererVersion);
	auto shader = pDevice->Create(Shit::ShaderCreateInfo{g_shaderSourceLanguage, shaderSource.size(), shaderSource.data()});
	...
	struct UBOAA
	{
		...
		atmosphereThickness};
	Shit::Buffer *ubobuffer = pDevice->Create(
		Shit::BufferCreateInfo{
			{},
			sizeof(UBOAA),
...
void generateOpticalDepthImage(
	Shit::Device *pDevice,
	float H0,
	Shit::Image *pDstImage2D,
	Shit::ImageLayout srcLayout,
	Shit::ImageLayout dstLayout)
{
	...
	auto descriptorSetLayout = pDevice->Create(
		Shit::DescriptorSetLayoutCreateInfo{static_cast<uint32_t>(bindings.size()), bindings.data()});
	std::vector<Shit::DescriptorPoolSize> poolSize{
		{Shit::DescriptorType::STORAGE_IMAGE, 1},
		{Shit::DescriptorType::UNIFORM_BUFFER, 1},
	};
	auto descriptorPool = pDevice->Create(
		Shit::DescriptorPoolCreateInfo{1, static_cast<uint32_t>(poolSize.size()), poolSize.data()});
...
//{
//	auto graphicsQueueFamilyIndex = pDevice->GetQueueFamilyIndexByFlag(Shit::QueueFlagBits::GRAPHICS_BIT, {});
//	auto longLiveCommandPool = pDevice->Create(
//		Shit::CommandPoolCreateInfo{{}, graphicsQueueFamilyIndex->index});
//	//auto _shortLiveCommandPool = pDevice->Create(
//	//	Shit::CommandPoolCreateInfo{Shit::CommandPoolCreateFlagBits::RESET_COMMAND_BUFFER_BIT, graphicsQueueFamilyIndex->index});
...
void FilterKernel::delta()
{
	_kernel2D.resize(1, 1);
}
