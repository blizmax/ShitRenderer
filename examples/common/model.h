...
public:
	Model(std::string_view name, std::string_view path, ResourceLoader *loader)
		: Resource(name, path, loader) {}
	~Model();

	Mesh *createMesh()
	{
		return _meshes.emplace_back(std::make_unique<Mesh>()).get();
	}
...
class ModelManager : public ResourceManager
{
	Resource *createImpl(
		...
		ParameterMap const &parameters) override;
