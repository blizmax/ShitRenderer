...
class Texture : public IdObject<Texture>
{
	...
	// std::unordered_map<Shit::ImageViewCreateInfo, Shit::ImageView *> _imageViews{};
...
public:
	...
	Texture(Shit::ImageCreateInfo const &imageCI, Shit::ImageViewType imageViewType, void *data);
	~Texture();
...
public:
	...
	Texture *createTexture(
		...
		Shit::ImageLayout imageLayout = Shit::ImageLayout::SHADER_READ_ONLY_OPTIMAL);
	Texture *createOrRetrieveTexture(
		Image const *image,
		Shit::ImageUsageFlagBits usage = Shit::ImageUsageFlagBits::SAMPLED_BIT,
		Shit::ImageLayout imageLayout = Shit::ImageLayout::SHADER_READ_ONLY_OPTIMAL);
	Texture *getTexture(std::string_view imageName);
	Texture *createTexture(Shit::ImageCreateInfo const &imageCI,
						   Shit::ImageViewType imageViewType, void *data);

	void removeTextureById(uint32_t id)
	{
		_textures.erase(id);
	}

	Texture *getTextureById(uint32_t id) const
	{
