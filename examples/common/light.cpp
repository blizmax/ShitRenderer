...
Light::Light(GameObject *parent, LightType lightType) : Component(parent)
{
	...
	getParentTransform()->addListener(
		std::bind(&Light::transformListener, this, std::placeholders::_1, std::placeholders::_2));
}
Light::~Light()
{
...
void Light::updateImpl()
{
	...
	case LIGHT_DISK_SINGLE_FACE:
		_ssboLight.radius = {
			glm::length(glm::mat3(_ssboLight.transformMatrix) * glm::vec3(1, 0, 0)),
			1,
...
void Light::updateUBOBuffer()
{
	...
	e.buffer->MapMemory(
		0, e.buffer->GetCreateInfoPtr()->size, (void **)&data);
