...
public:
	OctreeNodeBase(glm::vec3 p0, glm::vec3 p1, int depth)
		: _pmin(p0), _pmax(p1), _depth(depth) {}
...
protected:
	...
	// every four bits stand for a region ,value [0,7], if bit 3 == 0 mean no intersection
	virtual uint32_t getOccupyRegions(T const &p) const = 0;

public:
	OctreeNode(glm::vec3 p0, glm::vec3 p1, int depth)
		: OctreeNodeBase(p0, p1, depth) {}

	virtual void insert(T const &p, int maxDepth) = 0;

	constexpr decltype(auto) childCBegin() const
	{
		return _vals.cbegin();
	}
	constexpr decltype(auto) childCEnd() const
	{
		return _vals.cend();
	}
...
public:
	OctreeNodePoint(glm::vec3 p0, glm::vec3 p1, int depth)
		: OctreeNode(p0, p1, depth) {}
...
public:
	...
	void insertFace(
		size_t faceIndex, Primitive const *pPrimitive,
		std::vector<size_t> const &vertexIndices, int maxDepth);
