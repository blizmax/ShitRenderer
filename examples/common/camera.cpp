...
Camera::~Camera()
{
	for (auto e : _uboBuffers)
		g_App->getDevice()->Destroy(e);
	_uboBuffers.clear();

	for (auto e : _descriptorSets)
		g_App->getDescriptorPool()->Free(1, &e);
...
void Camera::prepareImpl()
{
	...
	auto frameCount = g_App->getFramesInFlight();
	...
	for (uint32_t i = 0; i < frameCount; ++i)
	{
		...
		bufferInfos[i] = {
			_uboBuffers[i], 0, offsetof(CameraUBO, cascadeSplit) + _cascadeSplits.size() * sizeof(float)};

		writes.emplace_back(
			_descriptorSets[i], DESCRIPTOR_BINDING_CAMERA, 0, 1, Shit::DescriptorType::STORAGE_BUFFER, nullptr, &bufferInfos[i]);
	};
...
Camera *Camera::setCameraSize(float cameraSize)
{
	...
	if (auto p = std::get_if<PerspectiveDescription>(&_extraDesc))
	{
		p->fovy = float(2.f * std::atan2(_focalLength/_cameraSize, 1. / (2 * p->aspect)));
	}
	updateProjectionMatrix();
	return this;
}
...
{
	return glm::inverse(getParentTransform()->getGlobalTransformMatrix());
}
void Camera::updateProjectionMatrix()
{
	if (auto p = std::get_if<PerspectiveDescription>(&_extraDesc))
	{
		...
		//								? glm::perspectiveRH_ZO(p->fovy, p->aspect, _near, _far)
		//								: glm::perspectiveRH_NO(p->fovy, p->aspect, _near, _far))
	...
	else
	{
		...
		//						 ? glm::orthoRH_ZO(-x, x, -y, y, _near, _far)
		//						 : glm::orthoRH_NO(-x, x, -y, y, _near, _far);
...
{
	if (!_uboBufferNeedUpdate[frameIndex])
		return;
...
EditCameraController::EditCameraController(GameObject *parent, Shit::Window *window)
	: Behaviour(parent), _window(window)
{
	_window->AddEventListener(std::bind(&EditCameraController::windowEventListener, this, std::placeholders::_1));
}
void EditCameraController::prepareImpl()
{
	getParentTransform()->translate({0., 0., 3.});
}
void EditCameraController::updatePerFrameImpl(float frameDtMs)
{
	...
	if (!io.WantCaptureMouse)
	{
		...
		// wheel
		auto keyStateMouseM = Shit::GetKeyState(Shit::KeyCode::KEY_MBUTTON);
		...
		if (keyStateMouseL.down) //|| (keyStateMouseL.down && keyStateAltL.down))
		{
			static float c = 0.001;
			_window->GetCursorPos(cursorX, cursorY);

			if (flag)
			{
				_window->GetCursorPos(preCursorX, preCursorY);
				flag = false;
			}
			...
			{
				auto a = glm::mat3(getParentTransform()->getGlobalTransformMatrix()) * glm::vec3((preCursorY - cursorY) * c, 0, 0);
				getParentTransform()->rotate(a, TransformSpace::PARENT);
				//getParentTransform()->pitch((preCursorY - cursorY) * c,TransformSpace::PARENT);
			}
			preCursorX = cursorX, preCursorY = cursorY;
		}
		else if (keyStateMouseR.down)
		{
			static float c = 0.001;
			...
			if (cursorX != preCursorX)
			{
				getParentTransform()->translate(
					glm::vec3((preCursorX - cursorX) * c, 0, 0), TransformSpace::LOCAL);
			}
			if (cursorY != preCursorY)
			{
				getParentTransform()->translate(
					glm::vec3(0, (cursorY - preCursorY) * c, 0), TransformSpace::LOCAL);
			}
			preCursorX = cursorX, preCursorY = cursorY;
		}
		else
...
void EditCameraController::windowEventListener(Shit::Event const &event)
{
	std::visit(Shit::overloaded{
				   [](auto &&) {},
				   [&](Shit::MouseWheelEvent const &ev)
				   {
					   if (!ImGui::GetIO().WantCaptureMouse)
					   {
						   ...
						   static float c = 0.1;
						   getParentTransform()->translate(
							   glm::vec3(0, 0, -glm::length(getParentTransform()->getGlobalPosition()) * c * ev.yoffset),
...
//	static float c = 0.1;
//	getParentTransform()->translate(
//		glm::vec3(0, 0, -glm::length(getParentTransform()->getGlobalPosition()) * c * yoffset),
//		TransformSpace::LOCAL);
//}