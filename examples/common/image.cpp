...
void ImageLoaderSTB::load(Resource *resource)
{
	...
	stbi_set_flip_vertically_on_load(pImage->isflipY());
	if (stbi_is_hdr_from_memory((unsigned char *)data.data(), len))
	{
		ret.data = (unsigned char *)stbi_loadf_from_memory(
			(unsigned char *)data.data(), len, &ret.width, &ret.height, &componentNum, requestComponentNum);
		ret.hdr = true;
...
{
	registerResourceLoader<ImageLoaderSTB>(DEFAULT_LOADER_NAME);
}
	...
	ParameterMap const &parameters)
{
	...
	if (it != parameters.end())
		requestComponentNum = std::atoi(it->second.c_str());
