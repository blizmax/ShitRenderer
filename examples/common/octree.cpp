...
float OctreeNodeBase::rayNodeIntersect(
	glm::vec3 const &o, glm::vec3 const &d) const
{
	...
	if (inPoint.x > 0)
		intersectRegions[0] |= 4;
	if (inPoint.y > 0)
		intersectRegions[0] |= 2;
	if (inPoint.z > 0)
		intersectRegions[0] |= 1;
...
{
	if (RayAABBIntersect(o, d, _pmin, _pmax) < 0)
		return -1;
...
void OctreeNodePoint::insert(glm::vec3 const &p, int maxDepth)
{
	...
	{
		pNode = static_cast<OctreeNodePoint *>((_childNodes[region] = std::make_unique<OctreeNodePoint>(p0, p1, _depth + 1)).get());
	}
...
uint32_t OctreeNodePoint::getOccupyRegions(glm::vec3 const &p) const
{
	...
	for (uint32_t i = 0; i < 8; ++i)
	{
		...
		if (glm::all(glm::greaterThanEqual(p, pmin)) && glm::all(glm::lessThanEqual(p, pmax)))
			return i;
	}
	return 0;
}
//================================
void OctreeNodeFace::getFaceInfo(
	size_t faceIndex, Primitive const *&pPrimitive, std::vector<size_t> &vertexOffset) const
{
...
uint32_t OctreeNodeFace::getOccupyRegions(
	Primitive const *pPrimitive, std::vector<size_t> const &vertexIndices) const
{
	uint32_t ret{};

	// get triangle aabb box
	glm::vec3 faceMinVal{FLT_MAX},
		faceMaxVal{-FLT_MAX};
...
void OctreeNodeFace::insertFace(
	size_t faceIndex, Primitive const *pPrimitive, std::vector<size_t> const &vertexIndices, int maxDepth)
{
	bool flag = false;
	if (_vals.size() < 10)
	{
...
float OctreeNodeFace::rayTriIntersect(glm::vec3 const &o, glm::vec3 const &d) const
{
	...
	for (auto e : _vals)
	{
		...
		t = RayTriIntersect(
				o, d,
				pPrimitive->positions[vertexOffset[0]],
				pPrimitive->positions[vertexOffset[1]],
...
float OctreeNodeFace::rayLeafIntersect(glm::vec3 const &o, glm::vec3 const &d) const
{
	return rayTriIntersect(o, d);
}
...
{
	_rootNode = std::make_unique<OctreeNodePoint>(
		_pMesh->boundingVolume.box.aabb.minValue,
		_pMesh->boundingVolume.box.aabb.maxValue,
		0);
...
{
	_rootNode = std::make_unique<OctreeNodeFace>(
		_pMesh,
		_pMesh->boundingVolume.box.aabb.minValue,
		_pMesh->boundingVolume.box.aabb.maxValue,
		0);
