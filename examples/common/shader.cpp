#include "shader.h"
ShaderSpvLoader::ShaderSpvLoader(Shit::Device *device)
	: _device(device)
{
}
void ShaderSpvLoader ::load(Resource *resource)
{
	...
	shaderRes->_setHandle(_device->Create(Shit::ShaderCreateInfo{
		Shit::ShadingLanguage::SPIRV,
		code.size(),
		code.data()}));

	shaderRes->close();
}
void ShaderSpvLoader::unload(Resource *resource)
{
	_device->Destroy(static_cast<Shader *>(resource)->getHandle());
}
//=========================
...
Resource *ShaderManager::createImpl(
	std::string_view path,
	ResourceLoader *loader,
