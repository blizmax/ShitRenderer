...
{
	_positionBuffer = g_App->getDevice()->Create(
	...
	{
		_normalBuffer = g_App->getDevice()->Create(
			Shit::BufferCreateInfo{
				{},
	...
	{
		_texcoordBuffer = g_App->getDevice()->Create(
			Shit::BufferCreateInfo{
				{},
	...
	{
		_indexBuffer = g_App->getDevice()->Create(
			Shit::BufferCreateInfo{
				{},
	...
	{
		_distanceAlongNormalBuffer = g_App->getDevice()->Create(
			Shit::BufferCreateInfo{
				{},
	...
	for (size_t len = _primitive->faceMaterialIds.size(); i < len; ++i)
	{
		if (_primitive->faceMaterialIds[i] != _primitive->faceMaterialIds[i - 1])
		{
			if (g_App->getMaterialDataBlockManager()->containsMaterialDataBlock(_primitive->faceMaterialIds[l]))
				materialDataBlock = g_App->getMaterialDataBlockManager()->getMaterialDataBlockById(_primitive->faceMaterialIds[l]);
			else
			...
			_drawCommands.emplace_back(
				materialDataBlock2,
				(uint32_t)_primitive->getFaceIndexOffset(l),
...
void PrimitiveRenderer::draw(Shit::CommandBuffer *cmdBuffer,
							 ...
							 uint32_t materialSetNumber)
{
	...
	cmdBuffer->BindVertexBuffers(Shit::BindVertexBuffersInfo{
		0,
		(uint32_t)std::size(buffers),
		buffers,
		offsets});
...
MeshRenderer::MeshRenderer(GameObject *parent, Mesh *mesh)
	: Component(parent), _mesh(mesh)
{
...
//=========================================
PrimitivePointRenderer::PrimitivePointRenderer(Primitive *primitive)
	: _primitive(primitive)
{
	_positionBuffer = g_App->getDevice()->Create(
		Shit::BufferCreateInfo{
...
PrimitivePointRenderer::~PrimitivePointRenderer()
{
	g_App->getDevice()->Destroy(_positionBuffer);
}
...
//==================
MeshPointRenderer::MeshPointRenderer(GameObject *parent, Mesh *mesh)
	: Component(parent), _mesh(mesh)
{
	for (auto &&e : _mesh->primitives)
	{
...
void OctreeMeshRenderer::getOctreeMeshPositions(
	OctreeNodeBase const *pNode, std::vector<glm::vec3> &pos)
{
	if (!pNode)
		return;
	glm::vec3 p0, p1;
...
void OctreeMeshRenderer::draw(Shit::CommandBuffer *cmdBuffer)
{
	...
	cmdBuffer->BindVertexBuffers(Shit::BindVertexBuffersInfo{
		...
		&_positionBuffer,
		offsets});
	cmdBuffer->Draw(Shit::DrawIndirectCommand{
		_drawCommand.vertexCount, 1, _drawCommand.startVertex, 0});
}