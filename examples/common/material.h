...
class MaterialDataBlock : public IdObject<MaterialDataBlock>
{
	...
	struct UBO
	{
		...
		//int normal_tex_index{-1};	 //norm. For normal mapping, use bump instead
...
public:
	...
	MaterialDataBlock(MaterialDataBlockManager *creator, std::string_view name)
		: _creator(creator), _name(name)
	{
	}
...
public:
	...
	bool containsMaterialDataBlock(uint32_t id)
	{
		return _materials.contains(id);
	}

	MaterialDataBlock *getMaterialDataBlockById() const
	{
		return _materials.at(_defaultMaterialId).get();
	};
	MaterialDataBlock *getMaterialDataBlockById(uint32_t id) const
	{
