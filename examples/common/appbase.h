...
protected:
	...
	// gui
	void initImGui();
	void renderImGui(uint32_t frameIndex);
	...
	void windowEventListener(Shit::Event const &event);
...
public:
	...
	constexpr uint32_t getFramesInFlight() const { return _swapchain->GetImageCount(); }
	...
	void addRecreateSwapchainListener(Shit::Slot<void()> slot)
	{
		_swapchainRecreateSignal.Connect(slot);
	}
	void removeRecreateSwapchainListener(Shit::Slot<void()> slot)
	{
		_swapchainRecreateSignal.Disconnect(slot);
	}
	...
	constexpr Shit::DescriptorSetLayout *getCameraDescriptorSetLayout() const
	{
		return _cameraDescriptorSetLayout;
	}
	constexpr Shit::DescriptorSetLayout *getMaterialDescriptorSetLayout() const
	{
		return _materialDescriptorSetLayout;
	}
	constexpr Shit::DescriptorSetLayout *getLightDescriptorSetLayout() const
	{
		return _lightDescriptorSetLayout;
	}
	constexpr Shit::DescriptorSetLayout *getEnvDescriptorSetLayout() const
	{
		return _envDescriptorSetLayout;
	}
	...
	virtual void recordDefaultCommandBuffer();
	...
	{
		return _samplers.at(std::string(name));
	}
};
