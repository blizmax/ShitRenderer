...
class Hello : public AppBase
{
	...
	Shit::ImageView *depthInputAttachmentImageView[MAX_FRAMES_IN_FLIGHT]{};
...
public:
	...
	void createDefaultRenderPass() override
	{
		...
		_defaultRenderTarget.clearValues.assign(
			_defaultRenderTarget.renderPass->GetCreateInfoPtr()->attachmentCount, clearColor);
	...
	void createDefaultRenderTarget() override
	{
		...
		auto depthFormat = _defaultRenderTarget.renderPass->GetCreateInfoPtr()->pAttachments[attachmentCount - 1].format;
		//================
		...
		for (int i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i)
		{
			...
			{
				colorImageViewCI.format = _defaultRenderTarget.renderPass->GetCreateInfoPtr()->pAttachments[1 + j].format;
	...
	void preparePipelines()
	{
		...
		readPipelineInfo.shaders = {
			...
			static_cast<Shader *>(_shaderManager->create(
				buildShaderPath(_device,
								_sampleCount == Shit::SampleCountFlagBits::BIT_1 ? readFragShaderPath : readMultisampleFragShaderPath,
								_rendererVersion),
		...
		std::vector<Shit::DescriptorSetLayout const *> tempSetLayouts(MAX_FRAMES_IN_FLIGHT, descriptorSetLayouts[0]);
	...
	{
		//
		Shit::DescriptorImageInfo imagesInfo[MAX_FRAMES_IN_FLIGHT * COLOR_ATTACHMENT_NUM];
		...
		for (uint32_t i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i)
		{
			...
			{
				imagesInfo[i * COLOR_ATTACHMENT_NUM + j] = Shit::DescriptorImageInfo{
					0,
					_defaultRenderTarget.imageViews[i * attachmentCount + j + 1],
					Shit::ImageLayout::SHADER_READ_ONLY_OPTIMAL};
		...
		{
			imagesInfo2[i] = {
				shadowSamplerLinear, shadowRenderTargets[i].imageViews[0],
				Shit::ImageLayout::SHADER_READ_ONLY_OPTIMAL};
			writes.emplace_back(Shit::WriteDescriptorSet{
