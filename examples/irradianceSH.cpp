...
static auto imagePath = SHIT_SOURCE_DIR "/assets/images/Arches_E_PineTree_3k.hdr";
...
public:
	void loadAssets()
	{
		...
		equirectangularTex->getImage()->GenerateMipmap(
			Shit::Filter::LINEAR,
	...
	void prepareSHLlmPipeline()
	{
		...
		irradianceSHLlmPipelineInfo.shaders = {
			static_cast<Shader *>(_shaderManager->create(
				buildShaderPath(_device, irradianceSHLlmShaderName, _rendererVersion), ResourceManager::DEFAULT_LOADER_NAME, {{"stage", "COMP"}}))};
		...
		irradianceSHLlmPipelineInfo.pipeline = _device->Create(Shit::ComputePipelineCreateInfo{
			shaderStageCI, pipelineLayout});
	}
	void prepareSHPipeline()
	{
		...
		irradianceSHPipelineInfo.shaders = {
			static_cast<Shader *>(_shaderManager->create(
				buildShaderPath(_device, irradianceSHShaderName, _rendererVersion), ResourceManager::DEFAULT_LOADER_NAME, {{"stage", "COMP"}}))};
		...
		irradianceSHPipelineInfo.pipeline = _device->Create(Shit::ComputePipelineCreateInfo{
			shaderStageCI, pipelineLayout});
	}
	...
	{
		Shit::DescriptorSetLayoutBinding binding =
	...
	void createCubemp()
	{
		...
		Shit::DescriptorImageInfo imageInfos[]{
			{sampler,
			 equirectangularTex->getImageView(),
			 Shit::ImageLayout::SHADER_READ_ONLY_OPTIMAL},
			{0,
	...
	{
		// create llm image
		irradianceSHLlmTex = _textureManager->createTexture(
			Shit::ImageCreateInfo{
				{},
		...
		Shit::WriteDescriptorSet llmwrites[]{
			{irradianceSHLlmPipelineInfo.descriptorSets[0],
			 0,
			 0,
			 1,
			 Shit::DescriptorType::COMBINED_IMAGE_SAMPLER,
		...
		Shit::WriteDescriptorSet writes[]{
			{irradianceSHPipelineInfo.descriptorSets[0],
			 0,
			 0,
			 1,
			 Shit::DescriptorType::STORAGE_IMAGE,
	...
	void generateIrradianceTex()
	{
		...
		executeOneTimeCommands(
			_device, [&](Shit::CommandBuffer *cmdBuffer)
			{
				...
				Shit::ImageMemoryBarrier imageBarrier{
				...
				imageBarrier = Shit::ImageMemoryBarrier{
					Shit::AccessFlagBits::SHADER_WRITE_BIT,
					Shit::AccessFlagBits::SHADER_READ_BIT,
					Shit::ImageLayout::GENERAL,
					Shit::ImageLayout::SHADER_READ_ONLY_OPTIMAL,
	...
	void writeSkyboDescriptorSet()
	{
		Shit::DescriptorImageInfo imageInfo{
			...
			cubeTex->getImageView(),
			Shit::ImageLayout::SHADER_READ_ONLY_OPTIMAL};
		Shit::WriteDescriptorSet writeSet{
